...
struct LibraryRecipe: Codable, Sendable {
    ...
    static let all: [LibraryRecipe] = [
        ...
        .init(modelID: "flux2-klein-4b", repository: "black-forest-labs/FLUX.2-klein-4B",
              include: ["model_index.json", "scheduler/", "transformer/", "vae/", "text_encoder/", "tokenizer/"],
              preparation: "完整目录下载后先使用 GPU；ANE 分区可在加速面板中按机型导出。"),
        ...
        .init(modelID: "minimax-h3-turbo", repository: "MiniMaxAI/MiniMax-H3", include: ["FL2VA/"],
              preparation: "默认仅下载 FL2VA（文生视频、首尾帧）。基础权重还需配套 LightX2V Turbo LoRA，并通过 prepare-lora 或创作请求生成合并缓存；参考视频另需 Ref2VA 的独立模型与 Turbo 来源记录。"),
        .init(modelID: "fastmetal-1.3b-qad", repository: "FastVideo/FastMetal-1.3B-QAD", include: ["model_index.json", "mlx_dit.json", "mlx_dit.safetensors", "scheduler/", "text_encoder/", "tokenizer/", "vae/"],
              preparation: "还需配置持久 Python/FastVideo worker 与 TAEHV。保留基础 mlx_dit 权重，下载前可取消 EMA 或其他非必要变体。")
    ]
}
