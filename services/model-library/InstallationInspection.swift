...
private struct InstallationProbe {
    ...
    mutating func run() {
        ...
        case "minimax-h3-turbo":
            ...
            } else { report.preparation.append("H3 Turbo 需要 LightX2V Turbo LoRA：运行 prepare-lora，或在创作请求中选择受支持的 LoRA 以生成合并缓存。参见 docs/USAGE.md。") }
        case "fastmetal-1.3b-qad":
            json("mlx_dit.json"); weights("mlx_dit.safetensors"); json("tokenizer/tokenizer.json")
            component("text_encoder"); component("vae")
            report.preparation.append("FastMetal 还需 Python/FastVideo 持久 worker 和解码器环境。按 profiles/fastmetal.example.json 配置模型目录内的 turbocider-fastmetal.json；文件检查不运行 Python，也不验证依赖或 TAEHV。")
