...
**LTX 2.5 纯 GPU 实测（2026-09-13）：** Apple M4 Max、64 GB 统一内存；
704×448、97 帧（约 4.04 秒）、24 FPS、11 步，关闭音频，命中文本条件缓存。
App 端到端生成耗时：**文生视频 63.08 秒，图生视频 62.35 秒**。
以上为已有单次记录，非中位数；未启用 ANE 或近似加速，使用分阶段权重释放。
