...
- 支持独立 VAE `ae.safetensors` 和独立 Qwen3-4B `qwen_3_4b.safetensors`；也支持 `TURBOCIDER_Z_IMAGE_VAE`、`TURBOCIDER_Z_IMAGE_LLM` 覆盖路径。
...
### 低内存 streaming/offload（2026-09-08 新增）

正式请求契约现支持 `residency: "streaming"` 或
`streaming_offload: true` 配合 `memory_budget_bytes`。运行时使用 pinned
sd.cpp 的 disk-backed parameter backend、mmap、layer streaming、max-vram 和
