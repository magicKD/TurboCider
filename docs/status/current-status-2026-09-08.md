...
- H3 BF16 SSD streaming 已加入预算驱动的动态 pinned-prefix：`memory_budget_bytes` 会按 activation reserve、两个 streaming slot 和完整 block payload 自动选择前缀；`h3_result`/运行 JSON 记录 pinned/streamed 数量和 I/O 遥测。无预算仍保持原有两槽路径，预算不足、前缀耗尽 streamed suffix 或显式前缀超预算均 fail closed。TurboCider Session 现在会保留 streamed/pinned DiT 与 conditioning，但 streamed 路线明确不保留 video/TAEH3 decoder，避免低内存请求把 VAE 一并常驻。真实 62 GiB Transformer 的同进程 retained probe 中，16 GiB 预算选择 14 pinned/36 streamed blocks，两次请求输出 SHA-256 完全一致；第二次 pinned denoise 11.109 s，对无预算 streamed 的 15.469 s，读取量减少 28.0%、等待减少 48.4%。完整 MP4 E2E 仍因本机非 Transformer fixture 的失效符号链接待补。
...
视频路径现已增加流式解码的 `tools/native/video_quality_gate.py`，不把完整 H3/LTX RGB 序列同时载入内存。现有 matched GPU 产物中，TurboCider 对 H3 direct 和 LTX direct 的 MP4 都逐文件一致；LTX GPU 对 GPU+ANE 的 704×448、97 帧输出 mean correlation `0.8498`、minimum correlation `0.7751`、mean MAE `21.59/255`，虽然 frame-to-frame motion energy 最大相对误差为 `8.04%`，仍未通过**对齐 RGB 回归门禁**。新增公开 Vision feature-print 抽样诊断为 mean distance `0.1862`、maximum `0.2079`，说明不能把 RGB 不一致直接等同于生成质量损坏，但该距离尚无多 prompt/seed 校准，也不能据此自动放行。因此 LTX `auto` 保持 GPU，不得把约 1.20× 的局部/历史加速写成已验收的正式 E2E hybrid。结构化证据见 `docs/design/validation/video-quality-gate-2026-09-08.json` 和 `docs/design/validation/vision-feature-print-2026-09-08.json`。
