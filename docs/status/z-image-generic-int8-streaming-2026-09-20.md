...
分支：`feature/z-image-generic-int8-streaming`，PR 基线：`feat/stream@85443b7`。性能实测基于 `8e2e97e` 的实现；两版上游之间只有设计文档变化，native 源码一致。旧实现基线为 `25b6d08` 的冻结 library。
...
上述 `make test` 使用默认发布构建。24 GiB 机器上，原有 `test_memory_constrained_routes_remain_plan_only_without_manifest` 固定请求 48 GiB，会因超出物理内存失败；这不是本次 streaming 改动引入的失败。性能样本保留原先带 test hooks 的冻结 library，不与后续发布构建混用。
...
原始工作区的 `local-experiments/z-image-generic-int8-20260920/` 保留正式请求、每张结果与哈希、进程采样、汇总、冻结 library、取消恢复记录和测试日志，不提交模型或生成产物。smoke 试跑、重复补丁及临时脚本已清理，相同图片合并存储并保留原路径链接。
