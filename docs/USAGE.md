...
固定 MLX C++ 0.32.0，完整 Xcode，Apple Silicon arm64。通过 `DEVELOPER_DIR`/`SDKROOT` 选已有编译器。当前固定官方 MLX wheel 的最低系统为 macOS 26.2，发行包按此声明；当前真实测试系统 macOS 26.6，其他系统尚待认证。`make setup` 使用 CPython 3.11 安装固定依赖到项目 `.venv` 和 App 自有转换工具链；其后构建不下载依赖，也不需要其他项目。输出 `dist/TurboCider.app` 和 `dist/cli/`，包含 MLX dylib/metallib；本机 ad-hoc 签名不等于 Developer ID 公证。模型保持在用户选择的原目录。
...
图像创作、图片插入/粘贴、有序参考、随机种子及模型 Load/Unload 的已实现范围与测试入口见 [Studio 实现记录](design/app-studio-implementation.md)。
