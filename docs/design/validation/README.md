...
Z-Image 性能分析必须先读取 `z-image-performance-ledger-2026-09-23.json`。
其中列出的 2026-09-22/重启前记录只可保留作 parity、实现历史或诊断证据，
不得再用于 kernel 选择、加速比、回归阈值或容量判断。当前权威结果是
...
- `z-image-gguf-streaming-2026-09-09.json`：修正为 CPU-staged diffusion 参数 backend 后的 GGUF streaming 证据，包含 Q3/Q4/Q8 256²、Q4 LoRA 256²、Q4 1024²两个 base seed、Q4 LoRA 1024²和 8/16 GiB probe，以及 shape/finite/近似质量和性能门禁。2026-09-08 的矩阵保留为旧 disk-backend 历史数据。
...
- `h3-ssd-pinned-prefix-retained-2026-09-08.json`：同一进程两次 retained-session H3 DiT probe。16 GiB pinned-prefix 选择 14/36 blocks；两次请求及 streamed/pinned 两条路线的 latent SHA-256 均一致。第二次 pinned request 为 11.109 s，对无预算 streamed 第二次 15.469 s，读取量减少 28.0%、等待减少 48.4%。该记录仍不包含完整 tokenizer/text encoder/VAE/MP4 E2E。
