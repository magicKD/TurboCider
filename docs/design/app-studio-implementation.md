...
- FLUX 文生图、单图修改、有序 1–8 图参考编辑。每次一个主结果、App 一个活动任务。后台不自动连发。
...
- 常用输出尺寸、原图保留强度；高级区支持 1–50 步、动态文本、常驻/分阶段释放、原有本机加速 profile。没有 CFG、LoRA、逐图权重等未被原生 schema 消费的假控件。
...
| `apps/macos/StudioState.swift` | 草稿、seed 策略、输入映射、序号/撤销、统一导入/粘贴 |
...
| `native/core/api.mm` | C ABI 加载/释放与资源互斥 |
| `native/models/flux.mm`、`native/backends/mlx.mm` | 图像权重 materialize 和会话资源释放；推理数学保持原样 |
...
- Studio 行为测试：种子、单输出、角色/顺序/撤销、输入快照、超限拒绝、粘贴去重、失败导入回滚、草稿恢复与真实步速计算。
...
H3/LTX 不进入本次正式生成验收；模型中心明确标为暂不可用。App 仍使用嵌入会话，退出运行中的 App 会提示中断，不宣称后台续跑。素材删除只移除绑定，持久副本保留以保护历史；自动垃圾回收、完整资产项目管理、视频编辑、批次和任务重排留后续。
