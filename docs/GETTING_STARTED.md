...
H3 and Wan have separate layout and dependency requirements; see the
...
Use development tools under `tools/coreml` to export FFN partitions offline
with Python and `coremltools`. Import the source manifest into the App to
compile and cache it natively; the App does not launch the Python exporter.
