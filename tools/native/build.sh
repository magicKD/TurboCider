...
"$CXX" -isysroot "$SDK" "${MACOS_FLAGS[@]}" "$LTX_OUT/ltx_video_finalizer_tool.o" "$LTX_OUT/libltx-runtime.a" "$OUT/audio.o" "$OUT/video.o" -o "$OUT/ltx-video-finalizer" -L"$MLX_ROOT/lib" -lmlx -ljaccl -licucore -framework Foundation -framework Metal -framework MetalPerformanceShaders -framework MetalPerformanceShadersGraph -framework AVFoundation -framework AudioToolbox -framework CoreMedia -framework CoreVideo -Wl,-rpath,"$MLX_ROOT/lib"
