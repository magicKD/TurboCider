#!/usr/bin/env python3
"""One exact GPU reference for a prepared Z hybrid prompt; not a speed campaign."""
import argparse
import ctypes as C
import hashlib
import json
...
request={'schema_version':2,'model':'z-image-turbo','operation':'image.generate',
 ...
 'execution':{'policy':'gpu','streaming':{'schema_version':1,'enabled':True,'selection':'manual','retention':'request',
 'stages':{'denoiser':{'residency':'streamed','block_group_size':1,'slot_count':2,'resident_prefix_blocks':1,'prefetch_distance':1,'io_workers':2}}}}}
config={'library':str(a.library.resolve()),'model_path':str(a.model.resolve()),'model_id':'z-image-turbo','constructor':'candidate','verify_streaming_sources':True}
plan={'scope':__doc__,'config':config,'request':request,'library_sha256':hashlib.sha256(a.library.read_bytes()).hexdigest(),
 ...
 'comparison':'Exploratory final latent/RGB error only; no release qualification or performance claim'}
