#!/usr/bin/env python3
"""Interleaved native MLX GGUF vs external sd.cpp reference benchmark.

The reference side always uses the pinned Unsloth stable-diffusion.cpp
``sd-server``.  The TurboCider side uses only native MLX GGUF.
Both sides stay resident
...
def direct_generate(base: str, payload: dict, output: Path) -> float:
    ...
    while True:
        ...
        # Keep reference completion-detection jitter small; the native side
        # does not poll a server.
        time.sleep(0.1)
...
def main() -> None:
    ...
    parser.add_argument("--variant", default="Q8_0")
    parser.add_argument("--library", type=Path, required=True)
    parser.add_argument("--server", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument(
        "--turbocider-backend", choices=("native-mlx",), default="native-mlx",
    ...
    # Create a temporary explicit model root; do not mutate the user's models
    # or inject runtime backend-selection environment variables.
    native_root = (args.native_component_root or component_root).resolve()
    native_layout = tempfile.TemporaryDirectory(prefix="tc-native-gguf-comparison-")
    native_model = Path(native_layout.name)
    for component_name in ("tokenizer", "split_files", "text_encoder", "vae"):
        source = native_root / component_name
        if source.exists():
            (native_model / component_name).symlink_to(source, target_is_directory=True)
    (native_model / gguf.name).symlink_to(gguf)
    lib = configure_library(library)
    engine, error = C.c_void_p(), C.c_void_p()
    status = lib.tc_engine_create_model(
        b"z-image-turbo-gguf", str(native_model / gguf.name).encode(), C.byref(engine), C.byref(error)
    )
    ...
    result = {
        ...
        "note": (
            ...
            "The reference uses sd-server; TurboCider uses native MLX GGUF only."
        ),
    }
