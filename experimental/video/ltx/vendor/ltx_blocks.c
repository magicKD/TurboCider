...
void ltx_native_free(ltx_native_denoiser *ctx) {
    ...
    for(unsigned i=0;i<8;++i)free(ctx->owned_strings[i]);
    free(ctx);
}
ltx_native_denoiser *ltx_native_create(const ltx_native_options *options,
    ...
    const char **fields[]={&ctx->options.checkpoint,&ctx->options.mlp_directories[0],&ctx->options.mlp_directories[1],&ctx->options.v2a_directories[0],&ctx->options.v2a_directories[1],&ctx->options.kv_directory,&ctx->options.qkv_directories[0],&ctx->options.qkv_directories[1]};
    for(unsigned i=0;i<8;++i)if(*fields[i]){ctx->owned_strings[i]=strdup(*fields[i]);if(!ctx->owned_strings[i]){snprintf(error,error_size,"option allocation failed");goto failed;}*fields[i]=ctx->owned_strings[i];}
    ...
    ctx->gpu=ltx_gpu_create("ltx_shaders.metal",error,error_size);if(!ctx->gpu)goto failed;
    if(options->parallel_av){ctx->audio_gpu=ltx_gpu_create("ltx_shaders.metal",error,error_size);if(!ctx->audio_gpu)goto failed;}
...
int ltx_native_run(ltx_native_denoiser *ctx,int stage,
    ...
    active_geometry=(generation_geometry){g->output_width,g->output_height,g->latent_frames,g->stage1_latent_height,g->stage1_latent_width,g->stage2_latent_height,g->stage2_latent_width,(uint32_t)g->stage1_video_tokens,(uint32_t)g->stage2_video_tokens,g->frames,g->output_height,g->output_width};
    ...
    if(ctx->options.kv_directory&&!attach_ane_kv_directory(gpu,ctx->weights,0,48,ctx->options.kv_directory,"int8_pc",&loaded_kv,error,error_size))goto cleanup;
    ...
    ltx_rng video_rng,audio_rng;ltx_rng_seed(&video_rng,ctx->options.seed+(stage==1?10000u:2u),0);ltx_rng_seed(&audio_rng,ctx->options.seed+2u,0);
    ok=run_denoise_schedule(gpu,ctx->weights,0,48,&rope,&workspace,ctx->io,ctx->conditioning,buffers[2],buffers[3],buffers[4],buffers[0],buffers[1],clean,prefix,strength,buffers[5],buffers[6],buffers[7],buffers[8],rows,audio_rows,text_rows,vp,ap,sigmas,count,stage==1,&video_rng,stage==1?&video_rng:&audio_rng,0,error,error_size);
...
cleanup:
    ...
    for(unsigned i=0;i<9;++i)ltx_gpu_buffer_free(buffers[i]);native_progress=NULL;native_opaque=NULL;av_parallel_audio_gpu=NULL;return ok;
}
