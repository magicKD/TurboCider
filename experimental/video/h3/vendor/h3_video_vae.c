...
typedef struct {
    ...
    uint32_t sequence;
    uint32_t batch;
    uint32_t patches_per_batch;
    uint32_t sequence_per_batch;
    int latent_h;
    int latent_w;
    int latent_t;
    int output_frames;
...
static int configure_shape(vae_context *vae, uint32_t batch,
                           char *error, size_t error_size) {
    size_t patches = (size_t)CHUNK_LATENT_TIME * (size_t)vae->latent_h *
                     (size_t)vae->latent_w;
    size_t sequence = patches + SUFFIX;
    if (!batch || patches > UINT32_MAX || sequence > UINT32_MAX ||
        patches > UINT32_MAX / batch || sequence > UINT32_MAX / batch) {
        fail(error, error_size, "video VAE batch shape is too large");
        return 0;
    }
    vae->batch = batch;
    vae->patches_per_batch = (uint32_t)patches;
    vae->sequence_per_batch = (uint32_t)sequence;
    vae->patches = (uint32_t)(patches * batch);
    vae->sequence = (uint32_t)(sequence * batch);
    return 1;
}

static int prepare_input_batch(vae_context *vae, const float *const *inputs,
    ...
    size_t row = 0;
    for (uint32_t batch = 0; batch < vae->batch; batch++) {
        const float *input = inputs[batch];
        if (!input) {
            free(rows);
            fail(error, error_size, "video VAE batch input is missing");
            return 0;
        }
        for (int time = 0; time < CHUNK_LATENT_TIME; time++) {
            ...
            for (int height = 0; height < vae->latent_h; height++)
                for (int width = 0; width < vae->latent_w; width++)
                    for (int channel = 0; channel < LATENT_CHANNELS;
                         channel++) {
                        size_t source = (((size_t)channel *
                            (size_t)vae->latent_t + (size_t)source_t) *
...
static int prepare_input(vae_context *vae, const float *input,
    ...
    return prepare_input_batch(vae, inputs, mean, deviation, error,
                               error_size);
}
...
static int prepare_rope(vae_context *vae, char *error, size_t error_size) {
    ...
    for (uint32_t batch = 0; batch < vae->batch; batch++) {
        uint32_t row = batch * vae->sequence_per_batch;
            ...
            for (int h = 0; h < vae->latent_h; h++)
                for (int w = 0; w < vae->latent_w; w++, row++) {
                    float axes[] = {
                        ...
                        2.0f * (((float)h + 0.5f) /
                                (float)vae->latent_h) - 1.0f,
                        2.0f * (((float)w + 0.5f) /
                                (float)vae->latent_w) - 1.0f
                    };
                    for (uint32_t axis = 0; axis < 3; axis++)
                        for (uint32_t frequency = 0; frequency < 8;
                             frequency++) {
                            float inverse = 1.0f /
        ...
        uint32_t end = (batch + 1) * vae->sequence_per_batch;
        while (row < end) {
...
#define OP(call, label) do {                                                    \
    ...
    OP(h3_gpu_sdpa_batch_f32(
        vae->gpu, vae->heads, vae->query, vae->key, vae->value,
        vae->batch, vae->sequence_per_batch, HEADS, HEAD_DIM,
...
static int pack_input_hidden(vae_context *vae, h3_gpu_tensor *zero,
                             char *error, size_t error_size) {
    for (uint32_t batch = 0; batch < vae->batch; batch++) {
        size_t patch_source = (size_t)batch * vae->patches_per_batch * HIDDEN;
        size_t hidden_base = (size_t)batch * vae->sequence_per_batch * HIDDEN;
        if (!gpu_op(vae, h3_gpu_copy_f32(
                vae->gpu, vae->hidden, hidden_base, vae->patch_hidden,
                patch_source, (size_t)vae->patches_per_batch * HIDDEN),
                error, error_size, "pack video VAE patches") ||
            !gpu_op(vae, h3_gpu_copy_f32(
                vae->gpu, vae->hidden,
                hidden_base + (size_t)vae->patches_per_batch * HIDDEN,
                vae->registers, 0, REGISTERS * HIDDEN), error, error_size,
                "pack video VAE registers") ||
            !gpu_op(vae, h3_gpu_copy_f32(
                vae->gpu, vae->hidden,
                hidden_base +
                    (size_t)(vae->patches_per_batch + REGISTERS) * HIDDEN,
                zero, 0, HIDDEN), error, error_size,
                "pack video VAE suffix")) return 0;
    }
    return 1;
}
...
#define OP(call, label) do {                                                    \
    ...
    OP(pack_input_hidden(vae, zero, error, error_size),
       "pack video VAE input sequence");
...
#define OP(call, label) do {                                                    \
    ...
    OP(pack_input_hidden(vae, zero, error, error_size),
       "pack tiled video VAE input sequence");
    for (int index = 0; index < LAYERS; index++)
...
static int unpack_frame_range_rows(vae_context *vae, const float *rows,
                                   uint32_t batch_index, int first_frame,
                                   int frame_count, h3_video_frames *output,
                                   char *error, size_t error_size) {
    if (first_frame < 0 || frame_count < 1 ||
        first_frame > vae->output_frames - frame_count ||
        batch_index >= vae->batch) {
    ...
    size_t row_base = (size_t)batch_index * vae->sequence_per_batch;
            ...
            for (int x = 0; x < pixel_w; x++) {
                ...
                for (int channel = 0; channel < 3; channel++) {
                    size_t component = ((((size_t)channel * 4 +
                        ...
                        (size_t)within_x);
                    float value = rows[(row_base + patch) * OUTPUT_PATCH +
                                       component] * deviation[channel] +
                                  mean[channel];
...
static float *read_projected(vae_context *vae, char *error,
                             size_t error_size) {
    size_t count = (size_t)vae->sequence * OUTPUT_PATCH;
    float *rows = malloc(count * sizeof(*rows));
    if (!rows) {
        fail(error, error_size, "out of memory reading video VAE patches");
        return NULL;
    }
    if (!h3_gpu_tensor_read_f32(vae->projected, rows, count)) {
        free(rows);
...
static int unpack_frame_range(vae_context *vae, int first_frame,
    ...
    if (!rows) return 0;
    int ok = unpack_frame_range_rows(vae, rows, 0, first_frame, frame_count,
                                     output, error, error_size);
    free(rows);
    return ok && h3_gpu_get_stats(vae->gpu, &output->gpu_stats);
}

static int unpack_batch_frames(vae_context *vae, h3_video_frames *outputs,
                               char *error, size_t error_size) {
    float *rows = read_projected(vae, error, error_size);
    if (!rows) return 0;
    int ok = 1;
    ...
    if (!h3_gpu_get_stats(vae->gpu, &stats)) ok = 0;
    for (uint32_t batch = 0; batch < vae->batch && ok; batch++) {
        memset(&outputs[batch], 0, sizeof(outputs[batch]));
        ok = unpack_frame_range_rows(
            vae, rows, batch, 0, vae->output_frames, &outputs[batch],
            error, error_size);
        if (ok) outputs[batch].gpu_stats = stats;
    }
    free(rows);
    if (!ok)
        for (uint32_t batch = 0; batch < vae->batch; batch++)
            h3_video_frames_free(&outputs[batch]);
    return ok;
}
...
static int configured_tile_pixels(int pixel_height, int pixel_width) {
    const char *value = h3_runtime_getenv("H3_VAE_TILE_PIXELS");
    if (value && *value) {
        char *end = NULL;
...
static int configured_tile_batch(int tile_count) {
    const char *value = h3_runtime_getenv("H3_VAE_TILE_BATCH");
    if (tile_count < 2) return 1;
    /* The common 512-class decoder geometry has four spatial tiles. A
     * two-tile batch removes half of the repeated MPSGraph submissions while
     * adding only one tile's activations. Same-latent decoder A/B tests keep
     * F32 RGB relative L2 below 4e-7. Other tile counts remain conservative
     * until they have their own matched performance measurements. */
    if (!value || !*value) return tile_count == 4 ? 2 : 1;
    char *end = NULL;
    long batch = strtol(value, &end, 10);
    if (!end || *end || batch < 1) return 1;
    if (batch > tile_count) batch = tile_count;
    while (batch > 1 && tile_count % batch) batch--;
    return (int)batch;
}
...
h3_video_vae_decoder *h3_video_vae_decoder_load(
    ...
    if (ok && h3_runtime_getenv("H3_PROFILE"))
    ...
    if (ok) {
        ...
        ok = configure_shape(vae, 1, error, error_size);
        if (ok)
...
static int decode_chunked(const char *weight_directory,
    ...
    int tile_batch = configured_tile_batch(tile_count);
    if (h3_runtime_getenv("H3_PROFILE"))
        fprintf(stderr,
                "h3: video VAE tiles %dx%d at %d pixels, batch %d\n",
                x_axis.count, y_axis.count, tile_pixels, tile_batch);
    vae_context vae;
    memset(&vae, 0, sizeof(vae));
    ...
    vae.latent_t = CHUNK_LATENT_TIME;
    vae.output_frames = FIRST_CHUNK_FRAMES;
    ok = configure_shape(&vae, (uint32_t)tile_batch, error, error_size);
    if (ok)
    ...
    for (int chunk = 0; chunk < chunks && ok; chunk++) {
        ...
        for (int base = 0; base < tile_count && ok; base += tile_batch) {
            const float **inputs = calloc((size_t)tile_batch,
                                          sizeof(*inputs));
            h3_video_frames *batch_frames = calloc(
                (size_t)tile_batch, sizeof(*batch_frames));
            if (!inputs || !batch_frames) {
                free(inputs);
                free(batch_frames);
                fail(error, error_size,
                     "out of memory batching video VAE tiles");
                ok = 0;
                break;
            }
            for (int batch = 0; batch < tile_batch && ok; batch++) {
                int index = base + batch;
                int tile_y = index / x_axis.count;
                int tile_x = index % x_axis.count;
                inputs[batch] = extract_latent_tile(
                    normalized_latent, latent_time, latent_height,
                    latent_width, chunk * 5,
                    y_axis.starts[tile_y] / SPATIAL_RATIO,
                    x_axis.starts[tile_x] / SPATIAL_RATIO,
                    CHUNK_LATENT_TIME, vae.latent_h, vae.latent_w,
                    error, error_size);
                if (!inputs[batch]) ok = 0;
            }
            free_tensor(&vae.latent);
            if (ok)
                ok = prepare_input_batch(
                         &vae, inputs, latent_mean, latent_std,
                         error, error_size) &&
                     run_resident_tile(&vae, error, error_size) &&
                     unpack_batch_frames(&vae, batch_frames,
                                         error, error_size);
            for (int batch = 0; batch < tile_batch; batch++)
                free((void *)inputs[batch]);
            free(inputs);
            if (ok) for (int batch = 0; batch < tile_batch; batch++) {
                tiles[base + batch] = batch_frames[batch].rgb;
                batch_frames[batch].rgb = NULL;
            }
            for (int batch = 0; batch < tile_batch; batch++)
                h3_video_frames_free(&batch_frames[batch]);
            free(batch_frames);
        }
        h3_video_frames decoded;
        memset(&decoded, 0, sizeof(decoded));
...
int h3_video_vae_decode(const char *weight_directory,
    ...
    if (!configure_shape(&vae, 1, error, error_size)) return 0;
