...
h3_terminal_protocol h3_terminal_detect(void) {
    if (h3_runtime_getenv("KITTY_WINDOW_ID") || h3_runtime_getenv("GHOSTTY_RESOURCES_DIR"))
        return H3_TERM_KITTY;
    const char *program = h3_runtime_getenv("TERM_PROGRAM");
    if ((program && !strcmp(program, "iTerm.app")) ||
        h3_runtime_getenv("ITERM_SESSION_ID") || h3_runtime_getenv("WEZTERM_PANE") ||
        h3_runtime_getenv("KONSOLE_VERSION"))
