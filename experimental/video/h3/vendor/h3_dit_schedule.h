...
/* Precompute only blocks selected by active_blocks. This is intended for a
 * previously validated fixed layer policy; NULL retains the full dynamic
...
double h3_dit_schedule_gate_score(const h3_dit_schedule *schedule,
                                  unsigned block);
/* Per-step mean absolute residual-gate magnitudes in video/text/audio order.
 * scores_count must equal steps * H3_DIT_MODALITIES. */
int h3_dit_schedule_gate_scores(const h3_dit_schedule *schedule,
                                unsigned block, double *scores,
                                size_t scores_count);
/* Per-step attention/MLP residual-gate magnitudes in
 * [step][video/text/audio][attention/MLP] order. */
