#!/usr/bin/env python3
"""Opt-in actual Core ML/Metal exercise of the verified bundle session path."""
import argparse
import json
...
with tempfile.TemporaryDirectory(prefix='tc-verified-session-') as raw:
    ...
    subprocess.run(['clang++','-std=c++20','-O2','-Wall','-Wextra','-Werror',
        ...
        str(ROOT/'tests/native/verified_hybrid_session_test.cpp'),'-L'+str(native),'-lturbocider',
