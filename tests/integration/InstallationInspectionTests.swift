...
@main struct InstallationInspectionTests {
    static func main() throws {
        ...
        try json("comfy/tokenizer/tokenizer.json", ["model": ["type": "BPE"]])
        try tensor("comfy/" + ZImageVariant.all[1].path)
        try tensor("comfy/split_files/vae/ae.safetensors")
        try tensor("comfy/split_files/text_encoders/qwen_3_4b.safetensors")
        try check(inspect(at: "comfy").status == "files_present", "INT8-only Comfy installation rejected")
        try fm.removeItem(at: root.appendingPathComponent("comfy/" + ZImageVariant.all[1].path))
        try tensor("comfy/" + ZImageVariant.all[2].path)
        try check(inspect(at: "comfy").status == "files_present", "NVFP4-only Comfy installation rejected")
        try check(ZImageVariant.all[1].include(sharedText: true) == [ZImageVariant.all[1].path, "split_files/vae/ae.safetensors"], "Shared text still downloaded")
        try check(ZImageVariant.all[0].include(sharedText: false).contains("tokenizer/tokenizer.json"), "Comfy tokenizer supplement omitted")
        ...
        try json("wan/mlx_dit.json", ["model_type": "fixture"]); try tensor("wan/mlx_dit.safetensors")
        try json("wan/tokenizer/tokenizer.json", ["model": ["type": "BPE"]])
        try tensor("wan/text_encoder/model.safetensors"); try tensor("wan/vae/taew2_1.safetensors")
        let wan = inspect("wan2.1-1.3b-qad", at: "wan")
        try check(wan.status == "needs_preparation", "Wan preparation requirements omitted")
        let roundtrip = try JSONDecoder().decode(InstallationInspection.self, from: JSONEncoder().encode(wan))
        try check(roundtrip.status == wan.status && roundtrip.checkedWeightFiles == 3, "CLI/App report roundtrip failed")
