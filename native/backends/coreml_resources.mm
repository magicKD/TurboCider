...
NSDictionary *coreml_resources(NSDictionary*request,const Event&event,std::atomic<bool>&cancelled){
 ...
 fs::path storage=support()/"coreml/flux2-klein-4b/m1088",cache=support()/"cache/coreml";int bucket=1088,ane_mlp_width=9216;
 ...
 if(action=="export"){
  ...
  return @{ @"action":@"export", @"source_manifest":@((storage/"manifest.json").c_str()), @"storage":@(storage.c_str()), @"log":@(log.c_str()), @"bucket":@(bucket), @"ane_mlp_width":@(ane_mlp_width) };
 }
 if(action=="inventory"){
