...
NSDictionary *coreml_resources(NSDictionary*request,const Event&event,std::atomic<bool>&cancelled){
 for(NSString*k in request)require([@[@"action",@"cache",@"manifest",@"source_manifest",@"storage",@"profile",@"model_root",@"python",@"python_path",@"kind",@"apply",@"plan_token"] containsObject:k],"unknown Core ML resource request field");
 ...
 fs::path storage=support()/"coreml/flux2-klein-4b/m1088",cache=support()/"cache/coreml";int bucket=1088;
 ...
 if(action=="export"){
  auto model=absolute(string_value(request,@"model_root"));require(fs::is_regular_file(model/"transformer/diffusion_pytorch_model.safetensors"),"FLUX safetensors model required");
  ...
  Dl_info info{};require(dladdr((void*)&coreml_resources,&info)!=0,"cannot locate bundled exporter");auto script=fs::path(info.dli_fname).parent_path()/"coreml/export_flux2.py";require(fs::is_regular_file(script),"bundled offline exporter missing");
  ...
  NSTask*task=[NSTask new];task.executableURL=[NSURL fileURLWithPath:@(python.c_str())];task.arguments=@[@(script.c_str()),@"--model",@(model.c_str()),@"--output",@(storage.c_str()),@"--bucket",@(std::to_string(bucket).c_str())];
  auto env=[NSProcessInfo.processInfo.environment mutableCopy];[env removeObjectForKey:@"PYTHONPATH"];[env removeObjectForKey:@"PYTHONHOME"];env[@"PYTHONNOUSERSITE"]=@"1";auto python_path=string_value(request,@"python_path",export_python_path);if(!python_path.empty())env[@"PYTHONPATH"]=@(python_path.c_str());env[@"PYTHONUNBUFFERED"]=@"1";task.environment=env;task.standardOutput=handle;task.standardError=handle;
  ...
  while(task.running){if(cancelled.load()){[task terminate];[task waitUntilExit];throw Cancelled();}int completed=0;try{if(fs::is_regular_file(storage/"progress.json"))completed=[read_json(storage/"progress.json")[@"completed"] intValue];}catch(...){}event("coreml_export",completed,20);std::this_thread::sleep_for(std::chrono::milliseconds(250));}
  require(task.terminationStatus==0,"Core ML export failed; inspect "+log.string());checkpoint(cancelled);require(fs::is_regular_file(storage/"manifest.json"),"export did not publish manifest");event("coreml_export",20,20);
  return @{@"action":@"export",@"source_manifest":@((storage/"manifest.json").c_str()),@"storage":@(storage.c_str()),@"log":@(log.c_str()),@"bucket":@(bucket)};
 }
 if(action=="inventory"){
