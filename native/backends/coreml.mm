...
CoreMLBranch::CoreMLBranch(const std::filesystem::path &path, int rows)
    : output_storage_(mx::contiguous(mx::zeros({1, rows, 3072}, mx::float16))), rows_(rows) {
    ...
    require(output_storage_.data_size() == size_t(rows) * 3072 &&
    ...
    output_ =
        [[MLMultiArray alloc] initWithDataPointer:output_storage_.data<mx::float16_t>()
                                         ...
                                         dataType:MLMultiArrayDataTypeFloat16
                                          strides:@[ @(rows * 3072), @1, @(rows * 3072), @3072 ]
                                      deallocator:^(void *) {
                                      }
                                            error:&error];
...
Tensor CoreMLBranch::predict(const Tensor &input, int actual) {
    ...
    MLMultiArray *in =
        [[MLMultiArray alloc] initWithDataPointer:(void *)input.data<mx::float16_t>()
                                            shape:@[ @1, @3072, @1, @(rows_) ]
                                         dataType:MLMultiArrayDataTypeFloat16
                                          strides:@[ @(rows_ * 3072), @1, @(rows_ * 3072), @3072 ]
                                      deallocator:^(void *) {
                                      }
                                            error:&error];
    ...
    if (actual_output.dataPointer != output_.dataPointer) {
        ...
        for (int row = 0; row < rows_; ++row)
            for (int c = 0; c < 3072; ++c) {
                ...
                ((uint16_t *)output_storage_.data<mx::float16_t>())[size_t(row) * 3072 + c] =
...
HybridSession::HybridSession(const std::filesystem::path &file, const std::filesystem::path &model,
    ...
    require([d[@"shape"][@"K"] intValue] == 3072 && [d[@"shape"][@"N"] intValue] == 3072,
    ...
    for (int i = 0; i < 20; ++i) {
        ...
        impl_->branches.push_back(std::make_unique<CoreMLBranch>(path, rows));
    }
    if (warmups) {
        auto input = mx::zeros({1, rows, 3072}, mx::float16);
        mx::eval(input);
        for (int iteration = 0; iteration < warmups; ++iteration)
            for (int block = 0; block < 20; ++block) {
                tc::checkpoint(cancelled);
                event("coreml_warmup", iteration * 20 + block, warmups * 20);
