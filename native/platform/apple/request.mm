...
Request request_from_json(NSDictionary *d) {
    ...
    if (version == 1) {
        ...
        auto descriptor = to_dictionary(module_for(r.model).describe());
        ...
        r.width = number(d, @"width", 512);
        r.height = number(d, @"height", 512);
        r.frames = number(d, @"frames", 1);
        r.steps = number(d, @"steps", 4);
        r.seed = number(d, @"seed", 42);
        r.fps = number(d, @"fps", 24);
        ...
        r.fps = number(output, @"fps", 24);
        r.audio = boolean(output, @"audio", true);
