...
Tensor Flux::encode(const Tokens&t,const Event&event,std::atomic<bool>&cancelled){
 ...
 for(int i=0;i<27;++i){checkpoint(cancelled);event("text_encode",i,27);std::string p="model.layers."+std::to_string(i);
  ...
  k=mx::repeat(k,4,1);v=mx::repeat(v,4,1);
 ...
 event("text_encode",27,27);return result;
}
}
