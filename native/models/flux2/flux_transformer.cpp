...
Tensor Flux::denoise(const Tensor &latent, const Tensor &text, float sigma, int height, int width,
    ...
    auto qkv = [&](const Tensor &a, const std::string &p, bool ctx) {
        auto q = heads(linear(a, w, p + (ctx ? ".add_q_proj" : ".to_q")), 24, 128),
             k = heads(linear(a, w, p + (ctx ? ".add_k_proj" : ".to_k")), 24, 128),
             v = heads(linear(a, w, p + (ctx ? ".add_v_proj" : ".to_v")), 24, 128);
    ...
    for (int i = 0; i < 20; ++i) {
        checkpoint(cancelled);
        event("transformer_block", 5 + i, 25);
        ...
        if (hybrid_) {
            ...
            auto q = rope_pairs(sqnorm(heads(parts[0], 24, 128), w.at(p + ".norm_q.weight"), 1e-5f),
                                cos, sin);
            auto k = rope_pairs(sqnorm(heads(parts[1], 24, 128), w.at(p + ".norm_k.weight"), 1e-5f),
                                cos, sin);
            auto att = attend(q, k, heads(parts[2], 24, 128));
            auto mlp = silu(parts[3]) * parts[4];
