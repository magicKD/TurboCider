...
Tensor Flux::encode_image(const Tensor &image, const Event &event, std::atomic<bool> &cancelled) {
    ...
    for (int block = 0; block < 4; ++block) {
        ...
        event("image_encode", block, 4);
        auto p = "encoder.down_blocks." + std::to_string(block);
        for (int i = 0; i < 2; ++i)
            x = ops.residual(x, p + ".resnets." + std::to_string(i));
        if (block < 3) {
    ...
    x = mx::reshape(mx::transpose(mx::reshape(x, {1, 32, h / 2, 2, w / 2, 2}), {0, 1, 3, 5, 2, 4}),
                    {1, 128, h / 2, w / 2});
    auto mean = mx::reshape(vae_.at("bn.running_mean"), {1, 128, 1, 1});
    auto std = mx::sqrt(mx::reshape(vae_.at("bn.running_var"), {1, 128, 1, 1}) +
                        Tensor(1e-4f, mx::bfloat16));
    x = (x - mean) / std;
    x = mx::transpose(mx::reshape(x, {1, 128, (h / 2) * (w / 2)}), {0, 2, 1});
    mx::eval(x);
    event("image_encode", 4, 4);
    return x;
}
} // namespace tc
