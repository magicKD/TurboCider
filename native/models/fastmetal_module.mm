...
public:
    ...
    NSDictionary* generate(const Request& request, const Event& event,
...
ModelModule fastmetal_module() {
    return {"fastmetal-1.3b-qad",
        [] { return Recipe{"fastmetal-1.3b-qad",{{"text_encode",{}},{"dit",{"text_encode"},3},{"video_vae",{"dit"}},{"export",{"video_vae"}}},true}; },
        [](const Request& request) { validate_fastmetal_request(request); },
        [](const std::filesystem::path& root) { return std::make_unique<FastMetalSession>(root); },
        [] { return @{ @"id": @"fastmetal-1.3b-qad", @"name": @"FastMetal 1.3B QAD",
                       @"executor": @YES, @"backend": @"mlx+ane-bridge",
                       @"operations": @[@"video.generate"], @"inputs": @[@"text"],
                       @"output": @"video", @"default_steps": @3, @"default_frames": @81,
                       @"default_width": @832, @"default_height": @480, @"default_fps": @16,
                       @"supports_lora": @YES, @"runtime_lora": @NO,
                       @"lora_mode": @"premerged-manifest",
                       @"request_lora_identity_validation": @YES,
                       @"supports_gpu_ane": @YES,
                       @"runtime_dependency": @"explicit-python-fastvideo-worker",
                       @"parallel_strategy": @"MLX GPU residual FFN + Core ML ANE FFN split",
                       @"candidate_limitations": @[@"Current paired latency meets the 5% direct-baseline gate; broader hardware medians remain pending",
                                                     @"LoRA requires a separately merged and provenance-verified checkpoint",
                                                     @"Premerged LoRA currently runs on GPU only; matching ANE artifacts remain pending"] }; }};
}
}
