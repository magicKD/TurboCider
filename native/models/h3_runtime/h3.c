...
void h3_cache_clear(h3_ctx *ctx) {
    ...
    free(ctx->dit_key);
    ctx->dit_key = NULL;
    free(ctx->dit_resident_key);
    ctx->dit_resident_key = NULL;
...
void h3_cache_get_info(const h3_ctx *ctx, h3_cache_info *info) {
    ...
    info->prepared_dit = ctx->dit != NULL && ctx->dit_key != NULL;
...
static int h3_key_environment(h3_key *key, const char *name) {
    const char *value = h3_runtime_getenv(name);
    return h3_key_append(key, "|env:%s=%zu:%s", name,
                         value ? strlen(value) : 0, value ? value : "");
}

static void h3_requested_condition_modalities(const h3_params *params,
                                              int ref2va,
                                              int *video, int *audio) {
    *video = 0;
    *audio = 0;
    if (!ref2va) {
        *video = params->first_frame != NULL || params->last_frame != NULL;
        return;
    }
    for (size_t index = 0; index < params->reference_count; index++) {
        ...
        case H3_REFERENCE_VIDEO:
            *video = 1;
            if (reference->include_embedded_audio) *audio = 1;
            break;
...
static char *h3_resident_key(const char *dit_path, const char *dit_index,
                             const h3_params *params, int ref2va,
                             int video_condition, int audio_condition) {
    static const char *environment[] = {
        "H3_DIT_LAYER_POLICY", "H3_DIT_GATE_SKIP", "H3_DIT_GATE_CACHE",
        "H3_DIT_STEP_GATE_SKIP",
        "H3_TOKEN_REDUCTION", "H3_TOKEN_REDUCTION_BLOCKS",
        "H3_TOKEN_REDUCTION_EARLY", "H3_TOKEN_REDUCTION_SCALE",
        "H3_FBC_THRESHOLD", "H3_FBC_MAX_HITS",
        "H3_TEACACHE_THRESHOLD", "H3_TEACACHE_RETAIN_STEPS",
        "H3_TEACACHE_COOLDOWN_STEPS", "H3_TEACACHE_MAX_HITS",
        "H3_TEACACHE_AUDIO_THRESHOLD",
        "H3_SOL_ATTN", "H3_SOL_TAU", "H3_SOL_DENSE_LAYERS",
        "H3_SOL_DENSE_STEPS", "H3_SOL_DENSE_FINAL_STEPS",
        "H3_SOL_FULL_SINK", "H3_SOL_BQ64_VERIFY",
        "H3_COREML_ANE_DIR", "H3_COREML_ANE_MODEL",
        "H3_COREML_ANE_BLOCK", "H3_COREML_ANE_BLOCKS",
        "H3_COREML_ANE_GPU_FC1", "H3_COREML_ANE_GPU_FC2",
        "H3_COREML_ANE_MANIFEST", "H3_COREML_ANE_INTERMEDIATE",
        "H3_COREML_ANE_OUTPUT_SCALE", "H3_COREML_LOAD_JOBS",
        "H3_COREML_USE_COMPILED", "H3_COREML_LAZY_LOAD",
        "H3_COREML_WARMUPS", "H3_COREML_SPECIALIZE_FAST",
        "H3_COREML_RESHAPE_FREQUENT", "H3_COREML_GPU_FALLBACK",
        "H3_COREML_FORCE_GPU_FALLBACK", "H3_COREML_FALLBACK_LATCH",
        "H3_BENCH_COREML_SAME_LOADED_AB",
        "H3_BENCH_COREML_CAPTURE_BLOCK",
        "H3_COREML_QKV_DIR", "H3_COREML_QKV_BLOCKS",
        "H3_COREML_QKV_HEADS",
        "H3_PRIVATE_ANE_MLP_DIR", "H3_PRIVATE_ANE_MLP_PRECISION",
        "H3_PRIVATE_ANE_MLP_FC2_CHUNKS",
        "H3_PRIVATE_ANE_QKV_DIR", "H3_PRIVATE_ANE_QKV_BLOCKS",
        "H3_PRIVATE_ANE_QKV_HEADS", "H3_PRIVATE_ANE_QKV_PRECISION",
        "H3_PRIVATE_ANE_QKV_CHECKPOINT", "H3_BENCH_COREML_QKV_AB",
        "H3_DISABLE_FUSED_MLP", "H3_DIT_F32_FINAL",
        "H3_DISABLE_DIT_ACTIVATION_ALIAS",
        "H3_DISABLE_FUSED_PATCH_CAST", "H3_DISABLE_FUSED_PATCH_PACK",
        "H3_SCALAR_PATCH", "H3_DISABLE_FUSED_FINAL_SLICE",
        "H3_DISABLE_FUSED_FINAL_HEAD", "H3_ZERO_COPY_WEIGHTS",
        "H3_NAX", "H3_NAX_FORCE", "H3_DISABLE_INT8_MLP",
        "H3_DISABLE_INT8_QKV", "H3_DISABLE_INT8_ATTENTION_OUT",
        "H3_INT8_KEEP_BF16_MLP", "H3_INT8_KEEP_BF16_QKV",
        "H3_INT8_KEEP_BF16_ATTENTION_OUT", "H3_INT8_MLP_STAGE"
    };
    h3_key key = {0};
    if (!h3_key_append(
            &key,
        ...
        !h3_key_file(&key, "transformer-index", dit_index)) goto failed;
    for (size_t index = 0;
         index < sizeof(environment) / sizeof(*environment); index++)
        if (!h3_key_environment(&key, environment[index])) goto failed;
    return key.text;
failed:
    free(key.text);
    return NULL;
...
static char *h3_prepared_key(const char *conditioning,
                             ...
                             int render_width, int render_height) {
    h3_key key = {0};
    const char *layer_policy = h3_runtime_getenv("H3_DIT_LAYER_POLICY");
    const char *gate_skip = h3_runtime_getenv("H3_DIT_GATE_SKIP");
    const char *gate_cache = h3_runtime_getenv("H3_DIT_GATE_CACHE");
    if (!layer_policy) layer_policy = "gate-ranked";
    if (!gate_skip) gate_skip = "dynamic";
    if (!gate_cache) gate_cache = "off";
    if (!h3_key_append(
            &key,
            ...
            params->steps, params->video_flow_shift,
            params->audio_flow_shift, params->dit_layers, params->core_reuse,
            params->token_reduction, params->use_int8_row_fc2,
            ...
            params->use_slower_dynamic_fc1_k,
            params->use_slower_grouped_quantizer,
            strlen(layer_policy), layer_policy, strlen(gate_skip), gate_skip,
            strlen(gate_cache), gate_cache)) {
        free(key.text);
        return NULL;
    }
    return key.text;
...
static int h3_dump_video_latent(h3_ctx *ctx, const float *values,
                                size_t elements, int time, int height,
                                int width) {
    const char *path = h3_runtime_getenv("H3_DUMP_VIDEO_LATENT");
    if (!path || !*path) return 1;
    if (!values || !elements || time < 1 || height < 1 || width < 1) {
        h3_set_error(ctx, "cannot dump an invalid video latent");
        return 0;
    }
    size_t spatial = (size_t)time * (size_t)height * (size_t)width;
    if (!spatial || elements % spatial || elements / spatial > UINT32_MAX) {
        h3_set_error(ctx, "video latent dump shape does not match its data");
        return 0;
    }
    FILE *stream = fopen(path, "wb");
    if (!stream) {
        h3_set_error(ctx, "cannot create video latent dump %s: %s", path,
                     strerror(errno));
        return 0;
    }
    static const char magic[8] = {'H', '3', 'L', 'A', 'T', 'F', '3', '2'};
    uint32_t shape[4] = {(uint32_t)(elements / spatial), (uint32_t)time,
                         (uint32_t)height, (uint32_t)width};
    uint64_t count = (uint64_t)elements;
    int ok = fwrite(magic, 1, sizeof(magic), stream) == sizeof(magic) &&
             fwrite(shape, sizeof(shape[0]), 4, stream) == 4 &&
             fwrite(&count, sizeof(count), 1, stream) == 1 &&
             fwrite(values, sizeof(*values), elements, stream) == elements;
    ...
    if (!ok) {
        h3_set_error(ctx, "cannot write video latent dump %s: %s", path,
                     strerror(saved_error ? saved_error : EIO));
        return 0;
    }
    fprintf(stderr,
            "h3: dumped video latent [%u,%u,%u,%u] F32 to %s\n",
            shape[0], shape[1], shape[2], shape[3], path);
    return 1;
}
...
static int h3_valid_params(h3_ctx *ctx, const h3_params *params) {
    ...
    if (!isfinite(params->video_flow_shift) ||
        !isfinite(params->audio_flow_shift) ||
        params->video_flow_shift <= 0.0 || params->audio_flow_shift <= 0.0) {
        h3_set_error(ctx, "video/audio flow shifts must be positive and finite");
        return 0;
    }
...
static int h3_parallel_prepare_requested(void) {
    const char *value = h3_runtime_getenv("H3_PARALLEL_PREPARE");
    return value && *value && strcmp(value, "0");
}

static void h3_parallel_prepare_release_request(
        h3_parallel_prepare *prepare) {
    if (!prepare) return;
    h3_text_embedding_free(&prepare->text);
    h3_layout_free(&prepare->layout);
}

static void *h3_parallel_prepare_main(void *opaque) {
    ...
    prepare->dit = h3_dit_load_t2va_core(
        ...
        prepare->use_slower_bf16_attention_output,
        prepare->use_slower_row_major_attention_output,
        prepare->use_slower_unfused_int8_inputs,
        prepare->use_slower_unfused_qkv_rope,
        prepare->use_slower_scalar_qkv_rms,
        prepare->use_slower_uncached_int8_scales,
        prepare->use_slower_dynamic_fc1_k,
        prepare->use_slower_grouped_quantizer,
        prepare->use_int8_row_fc2,
        NULL, NULL, prepare->error, sizeof(prepare->error));
    prepare->elapsed = h3_monotonic_seconds() - prepare->started_at;
    return NULL;
}

static int h3_parallel_prepare_start(
        h3_parallel_prepare *prepare, const char *dit_path,
        const h3_sigma_schedule *sigmas, const h3_params *params,
        int latent_t, int latent_h, int latent_w, int audio_t,
        int frame_count, float spatial_rope_scale,
        char *error, size_t error_size) {
    if (!prepare || !dit_path || !sigmas || !params) return 0;
    memset(prepare, 0, sizeof(*prepare));
    size_t elements = (size_t)H3_PARALLEL_PREPARE_TOKENS *
                      H3_TEXT_HIDDEN_SIZE;
    if (elements > SIZE_MAX / sizeof(*prepare->text.values)) return 0;
    prepare->text.values = calloc(elements, sizeof(*prepare->text.values));
    if (!prepare->text.values) return 0;
    prepare->text.tokens = H3_PARALLEL_PREPARE_TOKENS;
    prepare->text.width = H3_TEXT_HIDDEN_SIZE;
    h3_layout_spec spec = {
        H3_PARALLEL_PREPARE_TOKENS, latent_t, latent_h, latent_w,
        audio_t, frame_count, NULL, 0, NULL, 0
    };
    if (!h3_layout_build(&spec, &prepare->layout, error, error_size)) {
        h3_parallel_prepare_release_request(prepare);
        return 0;
    }
    prepare->dit_path = dit_path;
    prepare->sigmas = sigmas;
    prepare->active_blocks = (unsigned)params->dit_layers;
    ...
    prepare->use_slower_uncached_int8_scales =
        params->use_slower_uncached_int8_scales;
    prepare->use_slower_dynamic_fc1_k = params->use_slower_dynamic_fc1_k;
    prepare->use_slower_grouped_quantizer =
        params->use_slower_grouped_quantizer;
    prepare->use_int8_row_fc2 = params->use_int8_row_fc2;
    prepare->started_at = h3_monotonic_seconds();
    int status = pthread_create(
        &prepare->thread, NULL, h3_parallel_prepare_main, prepare);
    if (status != 0) {
        if (error && error_size)
            snprintf(error, error_size,
                     "cannot start parallel DiT prepare: %s",
                     strerror(status));
        h3_parallel_prepare_release_request(prepare);
        return 0;
    }
...
static h3_dit *h3_parallel_prepare_join(h3_parallel_prepare *prepare,
                                        char *error, size_t error_size) {
    if (!prepare || !prepare->started) return NULL;
    int status = pthread_join(prepare->thread, NULL);
    prepare->started = 0;
    h3_parallel_prepare_release_request(prepare);
    if (status != 0) {
        if (error && error_size)
            snprintf(error, error_size,
                     "cannot join parallel DiT prepare: %s",
                     strerror(status));
        return NULL;
    }
    h3_dit *dit = prepare->dit;
    prepare->dit = NULL;
    return dit;
}
...
h3_result *h3_generate(h3_ctx *ctx, const char *prompt,
    ...
    h3_parallel_prepare parallel_prepare;
    memset(&parallel_prepare, 0, sizeof(parallel_prepare));
    h3_dit *dit = NULL;
    h3_video_vae_decoder *preview_decoder = NULL;
    h3_taeh3_decoder *taeh3_decoder = NULL;
    ...
    char *dit_index = dit_path ? h3_path(
        dit_path, "model.safetensors.index.json") : NULL;
    ...
    if (!tokenizer_path || !text_path || !dit_path || !dit_index || !vae_path ||
        !audio_vae_path) {
        h3_set_error(ctx, "out of memory resolving generation model paths");
        goto cleanup;
    }
    const char *taeh3_weights = h3_runtime_getenv("H3_TAEH3_WEIGHTS");
    int use_taeh3 = taeh3_weights && *taeh3_weights;
    if (use_taeh3 && ref2va) {
        h3_set_error(ctx, "TAEH3 final decode is currently validated only "
                     "for FL2VA generation");
        goto cleanup;
    }
    ...
    int requested_video_condition = 0, requested_audio_condition = 0;
    h3_requested_condition_modalities(
        params, ref2va, &requested_video_condition, &requested_audio_condition);
    resident_key = h3_resident_key(
        dit_path, dit_index, params, ref2va,
        requested_video_condition, requested_audio_condition);
    if (!resident_key) {
        h3_set_error(ctx, "out of memory constructing resident-model cache key");
        goto cleanup;
    }
    ...
    if (use_taeh3) {
        h3_key taeh3_cache_key = {0};
        if (!h3_key_append(&taeh3_cache_key, "TAEH3|%dx%d",
                           latent_h, latent_w) ||
            !h3_key_file(&taeh3_cache_key, "weights", taeh3_weights)) {
            free(taeh3_cache_key.text);
            h3_set_error(ctx, "out of memory constructing TAEH3 cache key");
            goto cleanup;
        }
    ...
    if (ctx->cache_enabled && ctx->dit &&
        (!ctx->dit_resident_key ||
         strcmp(ctx->dit_resident_key, resident_key))) {
        h3_dit_free(ctx->dit);
        ctx->dit = NULL;
        free(ctx->dit_key);
        ctx->dit_key = NULL;
    ...
    if (ctx->cache_enabled && ctx->dit && ctx->dit_resident_key &&
        !strcmp(ctx->dit_resident_key, resident_key) &&
        (!ctx->dit_key || strcmp(ctx->dit_key, prepared_key))) {
        h3_dit_release_request(ctx->dit);
        free(ctx->dit_key);
        ctx->dit_key = NULL;
        if (h3_runtime_getenv("H3_PROFILE"))
            fprintf(stderr,
                    "h3: released stale DiT request state before conditioning\n");
    }
    conditioning_hit = ctx->cache_enabled && ctx->conditioning_key &&
        !strcmp(ctx->conditioning_key, conditioning_key);
    char detail[512] = {0};
    if (!h3_serving_schedule_build_shifts(
            params->steps, params->video_flow_shift,
            params->audio_flow_shift, &sigmas)) {
    ...
    if (parallel_prepare_eligible) {
        if (h3_parallel_prepare_start(
                &parallel_prepare, dit_path, &sigmas, params,
                temporal.video_t, latent_h, latent_w, temporal.audio_t,
                temporal.frame_count, spatial_rope_scale,
                detail, sizeof(detail))) {
            if (h3_runtime_getenv("H3_PROFILE"))
                fprintf(stderr,
                        "h3: parallel DiT prepare started during Qwen\n");
        } else if (h3_runtime_getenv("H3_PROFILE")) {
            fprintf(stderr,
                    "h3: parallel DiT prepare unavailable (%s); "
                    "using sequential load\n",
                    detail[0] ? detail : "initialization failed");
        }
    } else if (h3_parallel_prepare_requested() && h3_runtime_getenv("H3_PROFILE")) {
        fprintf(stderr,
                "h3: parallel DiT prepare ineligible for this request\n");
    }
    if (conditioning_hit) {
    ...
    } else if (ctx->cache_enabled && ctx->dit &&
               ctx->dit_resident_key &&
               !strcmp(ctx->dit_resident_key, resident_key)) {
        dit = ctx->dit;
        dit_is_cached = 1;
        if (!h3_dit_reprepare(
                dit, &text, &layout, &sigmas, spatial_rope_scale,
                condition_video_rows, condition_video_elements,
                condition_audio_rows, condition_audio_elements,
                h3_dit_progress_bridge, &progress, detail, sizeof(detail))) {
            fprintf(stderr,
                    "h3: resident DiT reprepare rejected (%s); "
            ...
            if (!key_copy) {
                ...
                ctx->dit_key = NULL;
                fprintf(stderr,
                        "h3: warning: could not retain prepared DiT key\n");
            } else {
                free(ctx->dit_key);
                ctx->dit_key = key_copy;
            }
            fprintf(stderr,
                    "h3: resident DiT cache hit; request state rebuilt\n");
        }
    }
    if (!dit && parallel_prepare.started) {
        dit = h3_parallel_prepare_join(
            &parallel_prepare, detail, sizeof(detail));
        if (dit && !h3_dit_reprepare(
                dit, &text, &layout, &sigmas, spatial_rope_scale,
                NULL, 0, NULL, 0,
                h3_dit_progress_bridge, &progress,
                detail, sizeof(detail))) {
            fprintf(stderr,
                    "h3: parallel DiT reprepare rejected (%s); "
                    "falling back to full reload\n",
                    detail[0] ? detail : "unknown error");
            h3_dit_free(dit);
            dit = NULL;
        } else if (dit && h3_runtime_getenv("H3_PROFILE")) {
            fprintf(stderr,
                    "h3: parallel DiT prepare joined load=%.3fs "
                    "overlap-wall=%.3fs\n",
                    parallel_prepare.elapsed,
                    h3_monotonic_seconds() - parallel_prepare.started_at);
        } else if (!dit && h3_runtime_getenv("H3_PROFILE")) {
            fprintf(stderr,
                    "h3: parallel DiT prepare failed (%s); "
                    "falling back to full reload\n",
                    parallel_prepare.error[0] ? parallel_prepare.error :
                                                detail);
        }
    }
    if (!dit && conditioned) {
    ...
    if (ctx->cache_enabled && !dit_is_cached) {
        ...
        if (!key_copy || !resident_copy) {
            free(key_copy);
            free(resident_copy);
            fprintf(stderr,
                    "h3: warning: could not retain resident DiT keys\n");
        } else {
            ctx->dit = dit;
            ctx->dit_key = key_copy;
            ctx->dit_resident_key = resident_copy;
            dit_is_cached = 1;
    ...
    if (!h3_dump_video_latent(ctx, video, video_count, temporal.video_t,
                              latent_h, latent_w))
        goto cleanup;
    if (dit_is_cached && h3_dit_final_evicted(dit)) {
        ctx->dit = NULL;
        free(ctx->dit_key);
        ctx->dit_key = NULL;
        free(ctx->dit_resident_key);
    ...
    if (use_taeh3) {
        ...
        if (ctx->cache_enabled && ctx->taeh3_decoder &&
            ...
            taeh3_is_cached = 1;
            fprintf(stderr, "h3: TAEH3 decoder cache hit\n");
        } else {
            ...
            if (taeh3_decoder && ctx->cache_enabled) {
                ...
                if (key_copy) {
                    ...
                    ctx->taeh3_decoder_key = key_copy;
                    taeh3_is_cached = 1;
                    fprintf(stderr,
                            "h3: TAEH3 decoder cache miss; retained\n");
                } else {
                    fprintf(stderr,
                            "h3: warning: could not retain TAEH3 key\n");
                }
            }
        }
        if (taeh3_decoder) {
            h3_progress_emit(&progress, "TAEH3 load", 1, 1);
            video_ok = h3_taeh3_decoder_decode(
                taeh3_decoder, video, temporal.video_t, &frames,
                detail, sizeof(detail));
        }
    } else if (preview_decoder) {
    ...
    if (params->output_path && *params->output_path) {
        ...
        if (!h3_ffmpeg_write_av_rgb24_f32(
                ...
                output_height, H3_FPS, h3_runtime_getenv("H3_OUTPUT_SILENT") ? NULL : waveform.pcm, waveform.samples,
    ...
    if (params->retain_decoded) {
        ...
        result->decoded_frames = frames.frames;
        result->decoded_rgb = frames.rgb;
        frames.rgb = NULL;
        result->audio_channels = waveform.channels;
        result->audio_samples = waveform.samples;
        result->audio_pcm = waveform.pcm;
        waveform.pcm = NULL;
    }
...
cleanup:
    ...
    free(decoder_key);
    free(taeh3_key);
    free(tokenizer_path); free(text_path); free(dit_path); free(dit_index);
    free(vae_path);
    free(audio_vae_path);
