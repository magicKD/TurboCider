...
static const char *ffmpeg_program(void) {
    const char *override = h3_runtime_getenv("H3_FFMPEG");
    return override && *override ? override : "ffmpeg";
}

static const char *ffprobe_program(void) {
    const char *override = h3_runtime_getenv("H3_FFPROBE");
...
int h3_ffmpeg_write_av_rgb24_f32(const char *path, const uint8_t *frames,
                                 ...
                                 char *error, size_t error_size) {
    /* A silent H3 request still needs a valid video container.  Reuse the
     * already-tested video-only writer instead of constructing an FFmpeg
     * command with a missing second pipe. */
    if (!pcm)
        return h3_ffmpeg_write_rgb24(
            path, frames, frame_count, width, height, fps,
            error, error_size);
    if (error && error_size) error[0] = '\0';
    if (!path || !*path || !frames || frame_count < 1 || width < 2 ||
