...
@property(nonatomic, strong) NSMutableDictionary<NSString *, H3SDPAOut *> *sdpaOutCache;
...
static MPSGraphTensorData *h3_gpu_graph_data(const h3_gpu_tensor *tensor,
    ...
    if (!stable || h3_runtime_getenv("H3_DISABLE_GRAPH_DATA_CACHE"))
...
static MPSCommandBuffer *h3_gpu_mps_command(H3GPU *gpu) {
    const char *override = h3_runtime_getenv("H3_REUSE_MPS_COMMAND");
...
static int h3_gpu_profile_enabled(void) {
    const char *value = h3_runtime_getenv("H3_PROFILE");
...
h3_gpu *h3_gpu_create(const char *shader_source_path,
    ...
    @autoreleasepool {
        ...
        gpu.sdpaOutCache = [NSMutableDictionary dictionary];
        ...
        if (h3_runtime_getenv("H3_DEBUG_GPU_MEMORY")) {
        ...
        if (![path isAbsolutePath]) {
            Dl_info location;
            if (dladdr((void *)&h3_gpu_create, &location) && location.dli_fname) {
                NSString *base = [@(location.dli_fname) stringByDeletingLastPathComponent];
        ...
        if (source) {
            ...
            const char *nax = h3_runtime_getenv("H3_NAX");
            BOOL m5 = [gpu.device.name rangeOfString:@"M5"].location !=
                      NSNotFound;
            const char *forceNAX = h3_runtime_getenv("H3_NAX_FORCE");
            BOOL forceTensorOps = forceNAX && *forceNAX &&
                                  strcmp(forceNAX, "0") != 0;
            BOOL wantsTensorOps =
                (m5 || forceTensorOps) &&
                (!nax || !*nax || strcmp(nax, "0") != 0);
            BOOL wantsSol = h3_runtime_getenv("H3_SOL_ATTN") != NULL;
            NSMutableDictionary<NSString *, NSString *> *macros =
                [NSMutableDictionary dictionary];
            if (wantsTensorOps) macros[@"H3_METAL_HAS_TENSOR"] = @"1";
            if (wantsSol) macros[@"H3_ENABLE_SOL_ATTN"] = @"1";
        ...
        NSMutableArray<NSString *> *names = [@[
            ...
            @"h3_grouped_qkv_rope_bf16_segment_coop",
            @"h3_grouped_qkv_rope_coreml_f16_segment_coop",
            @"h3_grouped_qkv_rope_ane_f32_segment_coop",
            @"h3_swiglu_bf16",
            ...
            @"h3_pack_coreml_f16", @"h3_add_coreml_f16_bf16",
            @"h3_pack_coreml_f16_transpose",
            @"h3_add_coreml_f16_bf16_transpose",
            @"h3_add_coreml_f16_bf16_transpose_checked",
            @"h3_ane_pack_bf16", @"h3_ane_unpack_bf16",
            @"h3_ane_unpack_bf16_rows_checked",
            @"h3_ane_unpack_bf16_rows_checked_range",
            @"h3_join_ane_linear_prefix_bf16_checked",
            @"h3_join_gpu_linear_suffix_bf16",
            @"h3_add_ane_f32_bf16_transpose_checked",
            @"h3_add_ane_f32_bf16_transpose_gate_adaln_checked",
            @"h3_add_ane_f32_bf16_transpose_gate_adaln_quantize_int8_checked",
            @"h3_add_ane_f32_bf16_transpose_gate_adaln_quantize_int8_checked_scalar",
            @"h3_relative_l1_residual_bf16",
            @"h3_relative_l1_bf16",
            @"h3_compare_bf16",
            @"h3_compare_ane_f32_bf16_prefix",
            ...
            @"h3_taeh3_input_bf16",
            @"h3_taeh3_temporal_concat_bf16",
            @"h3_taeh3_add_relu_bf16",
            @"h3_taeh3_upsample2_bf16",
            @"h3_taeh3_tgrow_bf16",
            @"h3_taeh3_output_f32"
        ] mutableCopy];
        if (h3_runtime_getenv("H3_SOL_ATTN")) {
            [names addObject:@"h3_sol_reduce_summaries_bf16"];
            [names addObject:@"h3_sol_thresholds_diag_bf16"];
            [names addObject:@"h3_sol_key_stats_bf16"];
            [names addObject:@"h3_sol_thresholds_stats_bf16"];
            [names addObject:@"h3_sol_route_mask_bf16"];
            [names addObject:@"h3_sol_attention_bf16"];
            if (gpu.tensorOpsEnabled)
                [names addObject:@"h3_sdpa_bf16_nax_bq64_all_exact"];
        }
        if (gpu.tensorOpsEnabled) {
...
static h3_gpu_tensor *h3_gpu_tensor_load_file(h3_gpu *opaque, const char *path,
    ...
    const char *zero_copy = h3_runtime_getenv("H3_ZERO_COPY_WEIGHTS");
...
static h3_gpu_tensor *h3_gpu_tensor_map_file_read_only(
                                              h3_gpu *opaque,
                                              const char *path,
                                              uint64_t file_offset,
                                              size_t elements,
                                              ...
                                              const char *label) {
    H3GPU *gpu = GPU(opaque);
    if (!gpu || !path || !*path || !elements || file_offset > INT64_MAX ||
        elements > SIZE_MAX / item_size) return NULL;
    size_t bytes = elements * item_size;
    if ((uint64_t)bytes > (uint64_t)INT64_MAX - file_offset) return NULL;
    size_t page = (size_t)getpagesize();
    if (file_offset % page) {
        h3_gpu_set_error(gpu,
                         @"read-only mapped %s offset is not page aligned",
                         label);
        return NULL;
    }
    int descriptor = open(path, O_RDONLY | O_CLOEXEC);
    ...
    if (stat_result != 0 || status.st_size < 0 ||
    ...
    void *mapping = mmap(NULL, bytes, PROT_READ, MAP_PRIVATE,
                         descriptor, (off_t)file_offset);
    int map_error = errno;
    close(descriptor);
    if (mapping == MAP_FAILED) {
        h3_gpu_set_error(gpu, @"cannot map %s payload from %s: %s", label,
    ...
    id<MTLBuffer> buffer = [gpu.device
        newBufferWithBytesNoCopy:mapping length:bytes
        options:MTLResourceStorageModeShared
        deallocator:^(void *pointer, NSUInteger length) {
            (void)pointer;
            (void)length;
...
h3_gpu_tensor *h3_gpu_tensor_map_bf16(h3_gpu *opaque, const char *path,
                                      uint64_t file_offset, size_t elements) {
    return h3_gpu_tensor_map_file_read_only(
        opaque, path, file_offset, elements, sizeof(uint16_t),
        H3_GPU_BF16, "BF16");
}
...
size_t h3_gpu_tensor_bytes(const h3_gpu_tensor *tensor) {
    return tensor ? TENSOR(tensor).bytes : 0;
}
...
int h3_gpu_tensor_read_f32_range(const h3_gpu_tensor *tensor,
                                 ...
                                 size_t elements) {
    if (!tensor || !values || TENSOR(tensor).readOnly ||
        TENSOR(tensor).dtype != H3_GPU_F32 ||
...
int h3_gpu_tensor_read_bf16(const h3_gpu_tensor *tensor, uint16_t *values,
                            size_t elements) {
    if (!tensor || !values || TENSOR(tensor).readOnly ||
        TENSOR(tensor).dtype != H3_GPU_BF16 ||
...
void *h3_gpu_tensor_host_pointer(h3_gpu_tensor *tensor) {
    return tensor ? TENSOR(tensor).buffer.contents : NULL;
}

/* Wrap page-aligned IOSurface storage without copying so Metal and the
 * experimental private ANE bridge share the same activation plane. */
h3_gpu_tensor *h3_gpu_tensor_wrap_f32(h3_gpu *opaque, void *base,
                                      size_t elements) {
    H3GPU *gpu = GPU(opaque);
    if (!gpu || !base || !elements ||
        elements > SIZE_MAX / sizeof(float)) return NULL;
    ...
    id<MTLBuffer> buffer = [gpu.device
        newBufferWithBytesNoCopy:base length:bytes
                     ...
                     deallocator:nil];
    if (!buffer) {
        h3_gpu_set_error(gpu, @"cannot wrap %zu shared ANE bytes", bytes);
        return NULL;
    }
    H3Tensor *tensor = [[H3Tensor alloc] init];
    tensor.elements = elements;
...
int h3_gpu_continue_releasing(h3_gpu *opaque,
                              ...
                              size_t tensor_count) {
    H3GPU *gpu = GPU(opaque);
    if (!gpu || !gpu.command || (tensor_count && !tensors)) return 0;
    @autoreleasepool {
        id<MTLCommandBuffer> next = [gpu.queue commandBuffer];
        if (!next) {
            h3_gpu_set_error(gpu,
                             @"cannot continue Metal release command chain");
            return 0;
        }
        NSMutableArray<H3Tensor *> *release =
            [NSMutableArray arrayWithCapacity:tensor_count];
        size_t release_bytes = 0;
        for (size_t index = 0; index < tensor_count; index++) {
            if (!tensors[index]) continue;
            H3Tensor *tensor = CFBridgingRelease(tensors[index]);
        ...
        stats.submissions++;
        stats.command_encode_seconds += commit_time - gpu.commandStartWall;
        stats.live_bytes = stats.live_bytes >= release_bytes ?
            stats.live_bytes - release_bytes : 0;
        gpu.stats = stats;
        gpu.command = next;
    }
    gpu.commandStartWall = h3_gpu_now();
...
typedef struct {
    uint32_t sequence, segment_heads, total_heads, head_offset;
    uint32_t head_dim, rope_half, block_index;
    float epsilon;
} qkv_segment_args;
typedef struct {
    qkv_segment_args qkv;
    uint32_t plane_rows;
} ane_qkv_segment_args;
...
int h3_gpu_linear_f32(h3_gpu *opaque, h3_gpu_tensor *output,
    ...
    if (!h3_runtime_getenv("H3_SCALAR_PATCH") && rows >= 16 && output_dim == 5376 &&
...
static H3SDPAOut *h3_gpu_sdpa_output_graph(H3GPU *gpu, uint32_t sequence,
                                           uint32_t heads,
                                           uint32_t head_dim,
    ...
    @autoreleasepool {
        NSString *cacheKey = [NSString stringWithFormat:
                              @"%u:%u:%u:%u:%.9g:%d",
                              sequence, heads, head_dim, output_dim, scale,
                              headMajor];
        H3SDPAOut *cached = gpu.sdpaOutCache[cacheKey];
        if (cached) return cached;
        MPSGraph *graph = [[MPSGraph alloc] init];
        NSArray<NSNumber *> *rowMajorShape =
        ...
        NSArray<NSNumber *> *weightShape =
            @[@1, @(output_dim), @(heads * head_dim)];
        NSArray<NSNumber *> *outputShape =
            @[@1, @(sequence), @(output_dim)];
        MPSGraphTensor *q = [graph placeholderWithShape:inputShape
        ...
        MPSGraphTensor *attention =
            [graph scaledDotProductAttentionWithQueryTensor:qt keyTensor:kt
                                                valueTensor:vt scale:scale
                                                       name:nil];
        MPSGraphTensor *rowMajor =
            [graph transposeTensor:attention dimension:1 withDimension:2
                              name:nil];
        MPSGraphTensor *flat = [graph reshapeTensor:rowMajor
                                          withShape:@[@1, @(sequence),
                                                      @(heads * head_dim)]
                                               name:nil];
        MPSGraphTensor *transposedWeight =
            [graph transposeTensor:weight dimension:1 withDimension:2
                              name:nil];
        MPSGraphTensor *projected =
            [graph matrixMultiplicationWithPrimaryTensor:flat
                                         secondaryTensor:transposedWeight
                                                    name:nil];
        H3SDPAOut *result = [[H3SDPAOut alloc] init];
        result.graph = graph;
        result.query = q;
        result.key = k;
        result.value = v;
        result.weight = weight;
        result.output = [graph castTensor:projected
                                   toType:MPSDataTypeBFloat16 name:nil];
        result.inputShape = inputShape;
        result.weightShape = weightShape;
        result.outputShape = outputShape;
        gpu.sdpaOutCache[cacheKey] = result;
        return result;
    }
}
...
int h3_gpu_sdpa_batch_f32(h3_gpu *opaque, h3_gpu_tensor *output,
...
int h3_gpu_sdpa_output_linear_bf16(
    ...
    gpu.headMajorSDPAInputs = NO;
    size_t inner = (size_t)heads * head_dim;
    size_t attention_count = (size_t)sequence * inner;
    size_t weight_count = (size_t)output_dim * inner;
    size_t output_count = (size_t)sequence * output_dim;
    if (!sequence || !heads || !head_dim || !output_dim ||
        inner > UINT32_MAX || !isfinite(scale) || !(scale > 0.0f) ||
        !h3_gpu_require_command(gpu) ||
        !h3_gpu_require_bf16(gpu, query, attention_count,
                             @"fused SDPA query") ||
        !h3_gpu_require_bf16(gpu, key, attention_count,
                             @"fused SDPA key") ||
        !h3_gpu_require_bf16(gpu, value, attention_count,
                             @"fused SDPA value") ||
        !h3_gpu_require_bf16(gpu, weight, weight_count,
                             @"fused SDPA output weight") ||
        !h3_gpu_require_bf16(gpu, output, output_count,
                             @"fused SDPA projected output")) return 0;
    H3SDPAOut *cache = h3_gpu_sdpa_output_graph(
        gpu, sequence, heads, head_dim, output_dim, scale, headMajor);
    if (!cache) return 0;
    @autoreleasepool {
        MPSCommandBuffer *command = h3_gpu_mps_command(gpu);
        MPSGraphTensorData *(^inputData)(const h3_gpu_tensor *) =
            ^MPSGraphTensorData *(const h3_gpu_tensor *tensor) {
                return h3_gpu_graph_data(tensor, cache.inputShape,
                                         MPSDataTypeBFloat16, 0);
            };
        NSDictionary *feeds = @{
            cache.query: inputData(query),
            cache.key: inputData(key),
            cache.value: inputData(value),
            cache.weight: h3_gpu_graph_data(weight, cache.weightShape,
                                             MPSDataTypeBFloat16, 1)
        };
        ...
        @try {
            ...
            h3_gpu_set_error(gpu, @"fused MPSGraph SDPA/output failed: %@",
                             exception.reason);
            return 0;
        }
        gpu.command = command.rootCommandBuffer;
...
typedef struct {
    uint32_t rows, heads, head_dim, blocks;
    uint32_t sink_start, sink_end, sink_q_start, sink_q_end;
    uint32_t head_major_input, head_major_output;
    float scale_log2, tau;
} h3_sol_args;
...
int h3_gpu_sol_sdpa_bf16(
    ...
    H3GPU *gpu = GPU(opaque);
    uint32_t headMajorInput = gpu.headMajorSDPAInputs ? 1u : 0u;
    gpu.headMajorSDPAInputs = NO;
    ...
    if (!rows || !heads || head_dim != 128 || !blocks ||
        !isfinite(scale) || !(scale > 0.0f) || !isfinite(tau) || tau < 0.0f ||
        sink_start > sink_end || sink_end > blocks ||
        sink_q_start > sink_q_end || sink_q_end > blocks ||
        !h3_gpu_require_command(gpu) ||
        !h3_gpu_require_bf16(gpu, query, elements, @"Sol query") ||
        !h3_gpu_require_bf16(gpu, key, elements, @"Sol key") ||
        !h3_gpu_require_bf16(gpu, value, elements, @"Sol value") ||
        !h3_gpu_require_bf16(gpu, output, elements, @"Sol output") ||
        !h3_gpu_require_elements(gpu, query_centroids, summary_elements,
                                 @"Sol query centroids") ||
        TENSOR(query_centroids).dtype != H3_GPU_F32 ||
        !h3_gpu_require_bf16(gpu, key_centroids, summary_elements,
                             @"Sol key centroids") ||
        !h3_gpu_require_bf16(gpu, value_sums, summary_elements,
                             @"Sol value sums") ||
        !h3_gpu_require_elements(gpu, thresholds, threshold_elements,
                                 @"Sol thresholds") ||
        TENSOR(thresholds).dtype != H3_GPU_F32 ||
        !h3_gpu_require_elements(gpu, routes, route_elements,
                                 @"Sol routes") ||
        TENSOR(routes).dtype != H3_GPU_F32) return 0;
    id<MTLComputePipelineState> reduce = h3_gpu_pipeline(
        gpu, @"h3_sol_reduce_summaries_bf16");
    id<MTLComputePipelineState> threshold = h3_gpu_pipeline(
        gpu, @"h3_sol_thresholds_diag_bf16");
    id<MTLComputePipelineState> route = h3_gpu_pipeline(
        gpu, @"h3_sol_route_mask_bf16");
    id<MTLComputePipelineState> attention = h3_gpu_pipeline(
        gpu, @"h3_sol_attention_bf16");
    if (!reduce || !threshold || !route || !attention ||
        reduce.maxTotalThreadsPerThreadgroup < 128 ||
        threshold.maxTotalThreadsPerThreadgroup < 128 ||
        route.maxTotalThreadsPerThreadgroup < 128 ||
        attention.maxTotalThreadsPerThreadgroup < 32) {
        h3_gpu_set_error(gpu, @"device cannot dispatch standalone Sol-Attn");
        return 0;
    }
    h3_sol_args args = {
        rows, heads, head_dim, blocks,
        sink_start, sink_end, sink_q_start, sink_q_end,
        headMajorInput, output_head_major ? 1u : 0u,
        scale * 1.4426950408889634f, tau
    };
    @autoreleasepool {
        id<MTLComputeCommandEncoder> encoder =
            [gpu.command computeCommandEncoder];
        ...
        [encoder setBuffer:TENSOR(key).buffer offset:0 atIndex:1];
        [encoder setBuffer:TENSOR(value).buffer offset:0 atIndex:2];
        [encoder setBuffer:TENSOR(key_centroids).buffer offset:0 atIndex:3];
        [encoder setBuffer:TENSOR(value_sums).buffer offset:0 atIndex:4];
        [encoder setBuffer:TENSOR(routes).buffer offset:0 atIndex:5];
        [encoder setBuffer:TENSOR(output).buffer offset:0 atIndex:6];
        [encoder setBytes:&args length:sizeof(args) atIndex:7];
        [encoder dispatchThreadgroups:MTLSizeMake((NSUInteger)rows * heads,
                                                  1, 1)
...
typedef struct {
    uint32_t rows, heads, blocks, sparse;
    float scale_log2;
} h3_nax_sdpa_args;
...
int h3_gpu_sdpa_bf16_nax_bq64_all_exact(
    ...
    gpu.headMajorSDPAInputs = NO;
    size_t elements = (size_t)rows * heads * head_dim;
    if (!gpu.tensorOpsEnabled || !rows || !heads || head_dim != 128 ||
        !isfinite(scale) || !(scale > 0.0f) ||
        !h3_gpu_require_command(gpu) ||
        !h3_gpu_require_bf16(gpu, query, elements,
                             @"TensorOps SDPA query") ||
        !h3_gpu_require_bf16(gpu, key, elements,
                             @"TensorOps SDPA key") ||
        !h3_gpu_require_bf16(gpu, value, elements,
                             @"TensorOps SDPA value") ||
        !h3_gpu_require_bf16(gpu, output, elements,
                             @"TensorOps SDPA output")) return 0;
    id<MTLComputePipelineState> pipeline = h3_gpu_pipeline(
        gpu, @"h3_sdpa_bf16_nax_bq64_all_exact");
    if (!pipeline || pipeline.maxTotalThreadsPerThreadgroup < 256) {
        h3_gpu_set_error(gpu,
                         @"device cannot dispatch BQ64 TensorOps SDPA");
        return 0;
    }
    h3_nax_sdpa_args args = {
        rows, heads, (rows + 63u) / 64u, 0,
    ...
    @autoreleasepool {
        ...
        [encoder dispatchThreadgroups:
            MTLSizeMake((NSUInteger)((rows + 63u) / 64u) * heads, 1, 1)
...
int h3_gpu_sol_sdpa_bf16_nax_bq64(
    ...
    size_t route_elements = threshold_elements * blocks;
    if (!gpu.tensorOpsEnabled || !rows || !heads || head_dim != 128 ||
        !blocks || !isfinite(scale) || !(scale > 0.0f) ||
        !isfinite(tau) || tau < 0.0f ||
        sink_start > sink_end || sink_end > blocks ||
        sink_q_start > sink_q_end || sink_q_end > blocks ||
        !h3_gpu_require_command(gpu) ||
        !h3_gpu_require_bf16(gpu, query, elements, @"BQ64 Sol query") ||
        !h3_gpu_require_bf16(gpu, key, elements, @"BQ64 Sol key") ||
        !h3_gpu_require_bf16(gpu, value, elements, @"BQ64 Sol value") ||
        !h3_gpu_require_bf16(gpu, output, elements, @"BQ64 Sol output") ||
        !h3_gpu_require_elements(gpu, query_centroids, summary_elements,
                                 @"BQ64 Sol query centroids") ||
        TENSOR(query_centroids).dtype != H3_GPU_F32 ||
        !h3_gpu_require_bf16(gpu, key_centroids, summary_elements,
                             @"BQ64 Sol key centroids") ||
        !h3_gpu_require_bf16(gpu, value_sums, summary_elements,
                             @"BQ64 Sol value sums") ||
        !h3_gpu_require_elements(gpu, thresholds, threshold_elements,
                                 @"BQ64 Sol thresholds") ||
    ...
    id<MTLComputePipelineState> threshold = h3_gpu_pipeline(
        gpu, @"h3_sol_thresholds_diag_bf16");
    id<MTLComputePipelineState> keyStats = h3_gpu_pipeline(
        gpu, @"h3_sol_key_stats_bf16");
    id<MTLComputePipelineState> thresholdStats = h3_gpu_pipeline(
        gpu, @"h3_sol_thresholds_stats_bf16");
    id<MTLComputePipelineState> route = h3_gpu_pipeline(
        gpu, @"h3_sol_route_mask_bf16");
    id<MTLComputePipelineState> attention = h3_gpu_pipeline(
        gpu, @"h3_sdpa_bf16_nax_bq64_all_exact");
    BOOL fastThreshold = route_elements >=
        (size_t)heads * head_dim * 2;
    if (!reduce || !threshold || !keyStats || !thresholdStats ||
        !route || !attention ||
        reduce.maxTotalThreadsPerThreadgroup < 128 ||
        threshold.maxTotalThreadsPerThreadgroup < 128 ||
        keyStats.maxTotalThreadsPerThreadgroup < 128 ||
        thresholdStats.maxTotalThreadsPerThreadgroup < 128 ||
        route.maxTotalThreadsPerThreadgroup < 128 ||
        attention.maxTotalThreadsPerThreadgroup < 256) {
        h3_gpu_set_error(gpu,
                         @"device cannot dispatch TensorOps BQ64 Sol-Attn");
        return 0;
    }
    h3_sol_args preprocess_args = {
        rows, heads, head_dim, blocks,
        sink_start, sink_end, sink_q_start, sink_q_end,
        1, 1, scale * 1.4426950408889634f, tau
    };
    h3_nax_sdpa_args attention_args = {
        rows, heads, blocks, 1, scale * 1.4426950408889634f
    };
    @autoreleasepool {
        id<MTLComputeCommandEncoder> encoder =
            [gpu.command computeCommandEncoder];
        ...
        [encoder setBuffer:TENSOR(key_centroids).buffer offset:0 atIndex:4];
        [encoder setBuffer:TENSOR(value_sums).buffer offset:0 atIndex:5];
        [encoder setBytes:&preprocess_args
                    length:sizeof(preprocess_args) atIndex:6];
        [encoder dispatchThreadgroups:MTLSizeMake(blocks, heads, 1)
                 threadsPerThreadgroup:MTLSizeMake(128, 1, 1)];
        [encoder endEncoding];
        ...
        if (fastThreshold) {
            encoder = [gpu.command computeCommandEncoder];
            [encoder setComputePipelineState:keyStats];
            [encoder setBuffer:TENSOR(key_centroids).buffer
                         offset:0 atIndex:0];
            [encoder setBuffer:TENSOR(routes).buffer offset:0 atIndex:1];
            [encoder setBytes:&preprocess_args
                        length:sizeof(preprocess_args) atIndex:2];
            [encoder dispatchThreadgroups:MTLSizeMake(heads, 1, 1)
                     threadsPerThreadgroup:MTLSizeMake(128, 1, 1)];
            [encoder endEncoding];

            encoder = [gpu.command computeCommandEncoder];
            [encoder setComputePipelineState:thresholdStats];
            [encoder setBuffer:TENSOR(query_centroids).buffer
                         offset:0 atIndex:0];
            [encoder setBuffer:TENSOR(routes).buffer offset:0 atIndex:1];
            [encoder setBuffer:TENSOR(thresholds).buffer offset:0 atIndex:2];
            [encoder setBytes:&preprocess_args
                        length:sizeof(preprocess_args) atIndex:3];
            [encoder dispatchThreadgroups:MTLSizeMake(blocks, heads, 1)
                     threadsPerThreadgroup:MTLSizeMake(128, 1, 1)];
            [encoder endEncoding];
        } else {
        ...
        [encoder setBuffer:TENSOR(routes).buffer offset:0 atIndex:3];
        [encoder setBytes:&preprocess_args
                    length:sizeof(preprocess_args) atIndex:4];
        [encoder dispatchThreadgroups:MTLSizeMake(blocks, heads, 1)
                 threadsPerThreadgroup:MTLSizeMake(128, 1, 1)];
        [encoder endEncoding];
        ...
        [encoder setBuffer:TENSOR(routes).buffer offset:0 atIndex:5];
        [encoder setBuffer:TENSOR(output).buffer offset:0 atIndex:6];
        [encoder setBytes:&attention_args
                    length:sizeof(attention_args) atIndex:7];
        [encoder dispatchThreadgroups:
            MTLSizeMake((NSUInteger)blocks * heads, 1, 1)
                 threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
        [encoder endEncoding];
    }
    h3_gpu_stats stats = gpu.stats;
    stats.direct_dispatches += fastThreshold ? 5 : 4;
    gpu.stats = stats;
    return 1;
}
...
static H3Conv *h3_gpu_conv2d_bf16_graph(
        H3GPU *gpu, uint32_t batch, uint32_t height, uint32_t width,
        uint32_t input_channels, uint32_t output_channels, uint32_t kernel,
        uint32_t padding, int has_bias, int relu) {
    @autoreleasepool {
        NSString *key = [NSString stringWithFormat:
            @"2b:%u:%u:%u:%u:%u:%u:%u:%d:%d", batch, height, width,
            input_channels, output_channels, kernel, padding, has_bias, relu];
        H3Conv *cached = gpu.convCache[key];
        if (cached) return cached;
        H3Conv *conv = [[H3Conv alloc] init];
        conv.graph = [[MPSGraph alloc] init];
        conv.inputShape = @[@(batch), @(height), @(width), @(input_channels)];
        conv.weightShape = @[@(output_channels), @(input_channels),
                             @(kernel), @(kernel)];
        conv.biasShape = @[@1, @1, @1, @(output_channels)];
        conv.outputShape = @[@(batch), @(height), @(width),
                             @(output_channels)];
        conv.input = [conv.graph placeholderWithShape:conv.inputShape
                                              dataType:MPSDataTypeBFloat16
                                                  name:nil];
        ...
        if (relu) result = [conv.graph reLUWithTensor:result name:nil];
        conv.output = result;
        gpu.convCache[key] = conv;
        return conv;
    }
...
int h3_gpu_conv2d_bf16(h3_gpu *opaque, h3_gpu_tensor *output,
                       const h3_gpu_tensor *input,
                       const h3_gpu_tensor *weight,
                       const h3_gpu_tensor *bias, uint32_t batch,
    ...
    H3GPU *gpu = GPU(opaque);
    if (!batch || !height || !width || !input_channels || !output_channels ||
        !kernel || kernel / 2 != padding || kernel % 2 == 0) return 0;
    size_t input_count =
        (size_t)batch * height * width * input_channels;
    size_t weight_count =
        (size_t)output_channels * input_channels * kernel * kernel;
    size_t output_count =
        (size_t)batch * height * width * output_channels;
    if (!h3_gpu_require_bf16(gpu, input, input_count, @"Conv2d input") ||
        !h3_gpu_require_bf16(gpu, weight, weight_count, @"Conv2d weight") ||
        !h3_gpu_require_bf16(gpu, output, output_count, @"Conv2d output") ||
        (bias && !h3_gpu_require_bf16(
            gpu, bias, output_channels, @"Conv2d bias")) ||
        !h3_gpu_require_command(gpu)) return 0;
    H3Conv *conv = h3_gpu_conv2d_bf16_graph(
        gpu, batch, height, width, input_channels, output_channels, kernel,
        padding, bias != NULL, relu != 0);
    if (!conv) return 0;
    @autoreleasepool {
        MPSCommandBuffer *command = h3_gpu_mps_command(gpu);
        NSMutableDictionary *feeds = [@{
            conv.input: h3_gpu_graph_data(input, conv.inputShape,
                                          MPSDataTypeBFloat16, 0),
            conv.weight: h3_gpu_graph_data(weight, conv.weightShape,
                                           MPSDataTypeBFloat16, 1)
        } mutableCopy];
        if (bias)
            feeds[conv.bias] = h3_gpu_graph_data(
                bias, conv.biasShape, MPSDataTypeBFloat16, 1);
        NSDictionary *results = @{
            conv.output: h3_gpu_graph_data(output, conv.outputShape,
                                            MPSDataTypeBFloat16, 0)
        };
        @try {
            [conv.graph encodeToCommandBuffer:command feeds:feeds
            ...
            h3_gpu_set_error(gpu, @"MPSGraph BF16 Conv2d failed: %@",
                             exception.reason);
            return 0;
        }
        gpu.command = command.rootCommandBuffer;
...
typedef struct {
    uint32_t time, height, width, channels;
} h3_taeh3_shape_args;

int h3_gpu_taeh3_input_bf16(h3_gpu *opaque, h3_gpu_tensor *output,
                            const h3_gpu_tensor *input, uint32_t time,
                            uint32_t height, uint32_t width,
                            uint32_t channels) {
    H3GPU *gpu = GPU(opaque);
    size_t elements = (size_t)time * height * width * channels;
    if (!time || !height || !width || !channels || elements > UINT32_MAX ||
        !h3_gpu_require_elements(gpu, input, elements, @"TAEH3 input") ||
        TENSOR(input).dtype != H3_GPU_F32 ||
        !h3_gpu_require_bf16(gpu, output, elements, @"TAEH3 BF16 input"))
        return 0;
    h3_taeh3_shape_args args = {time, height, width, channels};
    return h3_gpu_dispatch_1d(gpu, @"h3_taeh3_input_bf16",
        (uint32_t)elements, ^(id<MTLComputeCommandEncoder> encoder) {
            [encoder setBuffer:TENSOR(input).buffer offset:0 atIndex:0];
            [encoder setBuffer:TENSOR(output).buffer offset:0 atIndex:1];
            [encoder setBytes:&args length:sizeof(args) atIndex:2];
        });
...
int h3_gpu_taeh3_temporal_concat_bf16(
                            ...
                            uint32_t channels) {
    H3GPU *gpu = GPU(opaque);
    size_t input_elements = (size_t)time * height * width * channels;
    size_t output_elements = input_elements * 2;
    if (!time || !height || !width || !channels ||
        input_elements > SIZE_MAX / 2 || output_elements > UINT32_MAX ||
        !h3_gpu_require_bf16(gpu, input, input_elements, @"TAEH3 memory input") ||
        !h3_gpu_require_bf16(gpu, output, output_elements,
                             @"TAEH3 memory concat")) return 0;
    h3_taeh3_shape_args args = {time, height, width, channels};
    return h3_gpu_dispatch_1d(gpu, @"h3_taeh3_temporal_concat_bf16",
...
int h3_gpu_taeh3_add_relu_bf16(h3_gpu *opaque, h3_gpu_tensor *output,
                               const h3_gpu_tensor *residual,
                               const h3_gpu_tensor *branch,
                               uint32_t elements) {
    H3GPU *gpu = GPU(opaque);
    if (!elements ||
        !h3_gpu_require_bf16(gpu, residual, elements, @"TAEH3 residual") ||
        !h3_gpu_require_bf16(gpu, branch, elements, @"TAEH3 branch") ||
        !h3_gpu_require_bf16(gpu, output, elements, @"TAEH3 residual output"))
        return 0;
    return h3_gpu_dispatch_1d(gpu, @"h3_taeh3_add_relu_bf16", elements,
        ^(id<MTLComputeCommandEncoder> encoder) {
            [encoder setBuffer:TENSOR(residual).buffer offset:0 atIndex:0];
            [encoder setBuffer:TENSOR(branch).buffer offset:0 atIndex:1];
            [encoder setBuffer:TENSOR(output).buffer offset:0 atIndex:2];
...
int h3_gpu_taeh3_upsample2_bf16(h3_gpu *opaque, h3_gpu_tensor *output,
                                const h3_gpu_tensor *input, uint32_t time,
                                uint32_t height, uint32_t width,
                                uint32_t channels) {
    H3GPU *gpu = GPU(opaque);
    size_t input_elements = (size_t)time * height * width * channels;
    size_t output_elements = input_elements * 4;
    if (!time || !height || !width || !channels ||
        input_elements > SIZE_MAX / 4 || output_elements > UINT32_MAX ||
        !h3_gpu_require_bf16(gpu, input, input_elements, @"TAEH3 upsample input") ||
        !h3_gpu_require_bf16(gpu, output, output_elements,
                             @"TAEH3 upsample output")) return 0;
    h3_taeh3_shape_args args = {time, height, width, channels};
    return h3_gpu_dispatch_1d(gpu, @"h3_taeh3_upsample2_bf16",
        (uint32_t)output_elements,
        ^(id<MTLComputeCommandEncoder> encoder) {
            [encoder setBuffer:TENSOR(input).buffer offset:0 atIndex:0];
            [encoder setBuffer:TENSOR(output).buffer offset:0 atIndex:1];
...
typedef struct {
    uint32_t time, height, width, channels, stride;
} h3_taeh3_tgrow_args;

int h3_gpu_taeh3_tgrow_bf16(h3_gpu *opaque, h3_gpu_tensor *output,
                            const h3_gpu_tensor *input, uint32_t time,
                            uint32_t height, uint32_t width,
                            uint32_t channels, uint32_t stride) {
    H3GPU *gpu = GPU(opaque);
    size_t elements = (size_t)time * height * width * channels * stride;
    if (!time || !height || !width || !channels ||
        (stride != 1 && stride != 2) || elements > UINT32_MAX ||
        !h3_gpu_require_bf16(gpu, input, elements, @"TAEH3 TGrow input") ||
        !h3_gpu_require_bf16(gpu, output, elements, @"TAEH3 TGrow output"))
        return 0;
    h3_taeh3_tgrow_args args = {time, height, width, channels, stride};
    return h3_gpu_dispatch_1d(gpu, @"h3_taeh3_tgrow_bf16",
        (uint32_t)elements, ^(id<MTLComputeCommandEncoder> encoder) {
            [encoder setBuffer:TENSOR(input).buffer offset:0 atIndex:0];
            [encoder setBuffer:TENSOR(output).buffer offset:0 atIndex:1];
            [encoder setBytes:&args length:sizeof(args) atIndex:2];
...
typedef struct {
    uint32_t raw_time, height, width, frames;
} h3_taeh3_output_args;

int h3_gpu_taeh3_output_f32(h3_gpu *opaque, h3_gpu_tensor *output,
                            const h3_gpu_tensor *input, uint32_t raw_time,
                            uint32_t height, uint32_t width,
                            uint32_t frames) {
    H3GPU *gpu = GPU(opaque);
    size_t input_elements = (size_t)raw_time * height * width * 12;
    size_t output_elements = (size_t)frames * height * 2 * width * 2 * 3;
    uint32_t last = frames ?
        ((frames - 1) / 17) * 20 + 3 + (frames - 1) % 17 : 0;
    if (!raw_time || !height || !width || !frames || last >= raw_time ||
        output_elements > UINT32_MAX ||
        !h3_gpu_require_bf16(gpu, input, input_elements, @"TAEH3 RGB patches") ||
        !h3_gpu_require_elements(gpu, output, output_elements,
                                 @"TAEH3 RGB output") ||
        TENSOR(output).dtype != H3_GPU_F32) return 0;
    h3_taeh3_output_args args = {raw_time, height, width, frames};
    return h3_gpu_dispatch_1d(gpu, @"h3_taeh3_output_f32",
        (uint32_t)output_elements,
        ^(id<MTLComputeCommandEncoder> encoder) {
            [encoder setBuffer:TENSOR(input).buffer offset:0 atIndex:0];
            [encoder setBuffer:TENSOR(output).buffer offset:0 atIndex:1];
...
static int h3_gpu_linear_mps(H3GPU *gpu, h3_gpu_tensor *output,
    ...
    @autoreleasepool {
        ...
        @try {
            if (h3_runtime_getenv("H3_MPS_EXECUTABLE_LINEAR")) {
                if (!linear.executable) {
                    NSMutableDictionary *types = [NSMutableDictionary dictionary];
                    types[linear.input] = [[MPSGraphShapedType alloc]
                        initWithShape:linear.inputShape dataType:dataType];
                    types[linear.weight] = [[MPSGraphShapedType alloc]
                        initWithShape:linear.weightShape dataType:dataType];
                    if (bias)
                        types[linear.bias] = [[MPSGraphShapedType alloc]
                            initWithShape:linear.biasShape dataType:dataType];
                    linear.executable = [linear.graph compileWithDevice:
                        [MPSGraphDevice deviceWithMTLDevice:gpu.device]
                        feeds:types targetTensors:@[linear.output]
                        targetOperations:nil compilationDescriptor:nil];
                }
                NSMutableArray<MPSGraphTensorData *> *inputs =
                    [NSMutableArray array];
                for (MPSGraphTensor *tensor in linear.executable.feedTensors) {
                    MPSGraphTensorData *tensorData = feeds[tensor];
                    if (!tensorData) {
                        h3_gpu_set_error(gpu,
                            @"compiled MPS linear has an unknown feed");
                        return 0;
                    }
                    [inputs addObject:tensorData];
                }
                [linear.executable encodeToCommandBuffer:command
                    inputsArray:inputs resultsArray:@[output_data]
                    executionDescriptor:nil];
            } else {
...
int h3_gpu_linear_bf16(h3_gpu *opaque, h3_gpu_tensor *output,
    ...
    const char *splitRowsValue = h3_runtime_getenv("H3_NAX_SPLIT_ROWS");
...
static int h3_gpu_fc1_swiglu_nax_bf16(h3_gpu *opaque, h3_gpu_tensor *output,
    ...
    if (!gpu.tensorOpsEnabled ||
        ...
        !h3_gpu_require_command(gpu)) return 0;
    BOOL morton = h3_runtime_getenv("H3_DISABLE_NAX_MORTON") == NULL &&
                  h3_runtime_getenv("H3_DISABLE_NAX_MORTON_FC1") == NULL;
    uint32_t row_tiles = (rows + 127) / 128;
    uint32_t column_tiles = (hidden_dim + 63) / 64;
    BOOL morton4 = morton && !(column_tiles % 4) &&
                   h3_runtime_getenv("H3_DISABLE_NAX_MORTON4") == NULL &&
                   h3_runtime_getenv("H3_DISABLE_NAX_MORTON4_FC1") == NULL;
    id<MTLComputePipelineState> pipeline = h3_gpu_pipeline(
...
int h3_gpu_mlp_nax_bf16(h3_gpu *opaque, h3_gpu_tensor *output,
    ...
    BOOL morton = h3_runtime_getenv("H3_DISABLE_NAX_MORTON") == NULL &&
                  h3_runtime_getenv("H3_DISABLE_NAX_MORTON_FC2") == NULL;
    uint32_t row_tiles = (rows + 127) / 128;
    uint32_t column_tiles = (output_dim + 63) / 64;
    BOOL morton4 = morton && !(column_tiles % 4) &&
                   h3_runtime_getenv("H3_DISABLE_NAX_MORTON4") == NULL &&
                   h3_runtime_getenv("H3_DISABLE_NAX_MORTON4_FC2") == NULL;
    id<MTLComputePipelineState> pipeline = h3_gpu_pipeline(
        gpu, morton4 ? @"h3_linear_bf16_nax_r128_morton4" :
...
static int h3_gpu_quantize_bf16_int8_rows(
    ...
    BOOL vector_quantizer =
        ...
        h3_runtime_getenv("H3_INT8_VECTOR_QUANT");
...
static int h3_gpu_quantize_bf16_int8_groups(
    ...
    const char *quantizer_override = h3_runtime_getenv("H3_INT8_GROUP_QUANT_128");
    if (quantizer_override)
        scalar128 = *quantizer_override && strcmp(quantizer_override, "0");
    BOOL cached128 = scalar128 && group_size == 1024;
    const char *cache_override = h3_runtime_getenv("H3_INT8_GROUP_QUANT_128_CACHE");
    if (cache_override)
        cached128 = cached128 && *cache_override &&
            strcmp(cache_override, "0");
    NSString *kernel = h3_runtime_getenv("H3_INT8_VECTOR_QUANT") ?
...
static int h3_gpu_linear_int8_bf16_layout(
    ...
    if (!gpu.tensorOpsEnabled || rows < 128 || input_dim % 128 ||
        ...
        !h3_gpu_require_bf16(gpu, output, (size_t)rows * output_dim,
                             @"int8 linear output") ||
        !h3_gpu_require_command(gpu)) return 0;
    if (inputIsQuantized && headMajorInput) {
        h3_gpu_set_error(gpu,
                         @"prequantized head-major linear is unsupported");
        return 0;
    }
    if (!inputIsQuantized && headMajorInput) {
    ...
    } else if (!inputIsQuantized && !h3_gpu_quantize_bf16_int8_rows(
                   opaque, quantized_input, input_scales, input, rows,
                   padded_rows, input_dim, 1.0f,
                   @"int8 linear input")) return 0;
    BOOL local_scales = !use_slower_uncached_int8_scales &&
        h3_runtime_getenv("H3_DISABLE_INT8_LOCAL_SCALES") == NULL;
    BOOL known_linear = local_scales && rows <= 2048 && input_dim == 7168 &&
        output_dim == 5376 &&
        h3_runtime_getenv("H3_DISABLE_INT8_LINEAR_KNOWN") == NULL;
    id<MTLComputePipelineState> pipeline = h3_gpu_pipeline(
...
int h3_gpu_linear_int8_bf16_prequantized(
                            ...
                            int use_slower_uncached_int8_scales) {
    return h3_gpu_linear_int8_bf16_layout(
        opaque, output, (h3_gpu_tensor *)(void *)quantized_input,
        (h3_gpu_tensor *)(void *)input_scales, NULL, weight, weight_scales,
        rows, input_dim, output_dim, use_slower_uncached_int8_scales,
        YES, NO, 0, 0);
}
...
int h3_gpu_linear_int8_head_major_bf16(
    ...
    return h3_gpu_linear_int8_bf16_layout(
        opaque, output, quantized_input, input_scales, input, weight,
        weight_scales, rows, heads * head_dim, output_dim, 0, NO, YES,
        heads, head_dim);
}
...
int h3_gpu_mlp_int8_bf16(h3_gpu *opaque, h3_gpu_tensor *output,
    ...
    const char *stage = h3_runtime_getenv("H3_INT8_MLP_STAGE");
    ...
    const char *clip_value = h3_runtime_getenv("H3_INT8_ACTIVATION_CLIP");
    if (clip_value) {
    ...
    /* The local 128x64 kernel caches exactly 14 scale groups and pairs them
     * two at a time, so it is valid only for the production full-width
     * 14336-channel FC2. Sharded heterogeneous MLPs need the dynamic-stride
     * threadgroup kernel (or the explicitly requested local128 variant). */
    BOOL grouped_fc2_local = grouped_fc2 && hidden_dim == 14336u &&
        h3_runtime_getenv("H3_INT8_GROUP_FC2_THREADGROUP") == NULL;
    BOOL grouped_fc2_local128 = grouped_fc2 &&
        h3_runtime_getenv("H3_INT8_GROUP_FC2_LOCAL128") != NULL;
    BOOL int8_fc1_local = int8_fc1 &&
        h3_runtime_getenv("H3_INT8_FC1_LOCAL") != NULL;
    BOOL int8_fc1_known = int8_fc1 && input_dim == 5376 &&
        !use_slower_dynamic_fc1_k;
    const char *known_override = h3_runtime_getenv("H3_INT8_FC1_KNOWN");
    if (known_override)
        int8_fc1_known = int8_fc1 && input_dim == 5376 &&
            strcmp(known_override, "0") != 0;
    BOOL int8_fc1_full = int8_fc1_known &&
        h3_runtime_getenv("H3_DISABLE_FC1_FULL_K") == NULL;
...
int h3_gpu_gate_adaln_bf16(
    ...
    NSString *pipeline_name = h3_runtime_getenv("H3_DISABLE_EXACT_SIMD_GATE_ADALN") ?
        @"h3_gate_adaln_bf16" : @"h3_gate_adaln_bf16_exact_simd";
    id<MTLComputePipelineState> pipeline = h3_gpu_pipeline(
        gpu, pipeline_name);
    if (!pipeline) return 0;
...
int h3_gpu_gate_adaln_quantize_int8(
    ...
    NSString *pipeline_name = width == 5376 &&
        h3_runtime_getenv("H3_DISABLE_VECTOR_GATE_ADALN") == NULL ?
...
static int h3_gpu_qkv_rope_bf16_layout(h3_gpu *opaque, h3_gpu_tensor *query,
    ...
    if (head_dim == 128 && !(heads % 4) &&
        !h3_runtime_getenv("H3_DISABLE_COOP_QKV")) {
        if (!h3_gpu_require_command(gpu)) return 0;
        NSString *pipeline_name = h3_runtime_getenv("H3_DISABLE_CACHED_QKV") ?
            @"h3_qkv_rope_bf16_coop_uncached" :
...
static int h3_gpu_grouped_qkv_rope_segment_common(
                                 h3_gpu *opaque,
                                 h3_gpu_tensor *query,
                                 h3_gpu_tensor *key,
                                 h3_gpu_tensor *value,
                                 ...
                                 int source_kind, uint32_t plane_rows) {
    H3GPU *gpu = GPU(opaque);
    size_t segment_inner = (size_t)segment_heads * head_dim;
    size_t total_inner = (size_t)total_heads * head_dim;
    size_t source_rows = source_kind == 2 ? plane_rows : sequence;
    size_t segment_count = source_rows * segment_inner;
    size_t total_count = (size_t)sequence * total_inner;
    size_t rope_count = (size_t)sequence * rope_half;
    if (!sequence || !segment_heads || !total_heads || !head_dim ||
        (source_kind == 2 && plane_rows < sequence) ||
        head_offset > total_heads || segment_heads > total_heads - head_offset ||
        rope_half * 2 > head_dim || head_dim != 128 ||
        !h3_gpu_require_elements(gpu, qkv, segment_count * 3,
                                 @"segmented QKV input") ||
        !h3_gpu_require_bf16(gpu, q_norm, head_dim, @"Q norm") ||
        !h3_gpu_require_bf16(gpu, k_norm, head_dim, @"K norm") ||
        !h3_gpu_require_bf16(gpu, rope_cos, rope_count, @"RoPE cosine") ||
        !h3_gpu_require_bf16(gpu, rope_sin, rope_count, @"RoPE sine") ||
        !h3_gpu_require_bf16(gpu, query, total_count, @"query") ||
        !h3_gpu_require_bf16(gpu, key, total_count, @"key") ||
        !h3_gpu_require_bf16(gpu, value, total_count, @"value")) return 0;
    if ((source_kind == 0 && TENSOR(qkv).dtype != H3_GPU_BF16) ||
        (source_kind == 1 && TENSOR(qkv).dtype != H3_GPU_F16) ||
        (source_kind == 2 && TENSOR(qkv).dtype != H3_GPU_F32)) {
        h3_gpu_set_error(gpu,
            source_kind == 1 ? @"Core ML segmented QKV input is not F16" :
            source_kind == 2 ? @"private ANE segmented QKV input is not F32" :
                               @"GPU segmented QKV input is not BF16");
        return 0;
    }
    if (source_kind != 0 &&
        (!h3_gpu_require_elements(gpu, nonfinite_flag, 1,
                                  @"segmented QKV nonfinite flag") ||
         TENSOR(nonfinite_flag).dtype != H3_GPU_F32)) {
        h3_gpu_set_error(gpu,
                         @"segmented QKV nonfinite flag is not F32");
        return 0;
    }
    if (!h3_gpu_require_command(gpu)) return 0;
    NSString *name = source_kind == 1 ?
        @"h3_grouped_qkv_rope_coreml_f16_segment_coop" :
        source_kind == 2 ?
        @"h3_grouped_qkv_rope_ane_f32_segment_coop" :
        @"h3_grouped_qkv_rope_bf16_segment_coop";
    ...
    if (!pipeline || pipeline.maxTotalThreadsPerThreadgroup < head_dim) {
        h3_gpu_set_error(gpu,
                         @"segmented QKV/RoPE needs 128 threads");
        return 0;
    }
    qkv_segment_args args = {
        sequence, segment_heads, total_heads, head_offset,
        head_dim, rope_half, block_index, epsilon
    };
    ane_qkv_segment_args ane_args = {args, plane_rows};
    @autoreleasepool {
        id<MTLComputeCommandEncoder> encoder =
            [gpu.command computeCommandEncoder];
        [encoder setComputePipelineState:pipeline];
        ...
        [encoder setBuffer:TENSOR(value).buffer offset:0 atIndex:7];
        if (source_kind != 0)
            [encoder setBuffer:TENSOR(nonfinite_flag).buffer
                         offset:0 atIndex:8];
        if (source_kind == 2)
            [encoder setBytes:&ane_args length:sizeof(ane_args) atIndex:9];
        else
            [encoder setBytes:&args length:sizeof(args)
                       atIndex:source_kind == 1 ? 9 : 8];
        [encoder dispatchThreadgroups:
            MTLSizeMake((segment_heads + 3u) / 4u, sequence, 1)
                 threadsPerThreadgroup:MTLSizeMake(head_dim, 1, 1)];
        [encoder endEncoding];
    }
    h3_gpu_stats stats = gpu.stats;
...
int h3_gpu_grouped_qkv_rope_bf16_segment(
                                 ...
                                 uint32_t head_dim, uint32_t rope_half,
                                 float epsilon) {
    return h3_gpu_grouped_qkv_rope_segment_common(
        gpu, query, key, value, qkv, q_norm, k_norm, rope_cos, rope_sin,
        NULL, sequence, segment_heads, total_heads, head_offset, head_dim,
        rope_half, epsilon, 0, 0, sequence);
}
...
int h3_gpu_grouped_qkv_rope_coreml_f16_segment(
    ...
    return h3_gpu_grouped_qkv_rope_segment_common(
        gpu, query, key, value, qkv, q_norm, k_norm, rope_cos, rope_sin,
        nonfinite_flag, sequence, segment_heads, total_heads, head_offset,
        head_dim, rope_half, epsilon, block_index, 1, sequence);
}
...
int h3_gpu_grouped_qkv_rope_ane_f32_segment(
    ...
    return h3_gpu_grouped_qkv_rope_segment_common(
        gpu, query, key, value, qkv, q_norm, k_norm, rope_cos, rope_sin,
        nonfinite_flag, sequence, segment_heads, total_heads, head_offset,
        head_dim, rope_half, epsilon, block_index, 2, plane_rows);
}
...
int h3_gpu_grouped_qkv_linear_rope_bf16(
    ...
    BOOL candidate = gpu.tensorOpsEnabled && rows >= 64 && rows <= 2048 &&
        ...
        (gpu.tensorOpsMode == 1 || gpu.tensorOpsMode == 3 ||
         gpu.tensorOpsMode == 4) &&
        !h3_runtime_getenv("H3_DISABLE_NAX_LINEAR") &&
        !h3_runtime_getenv("H3_DISABLE_SPLIT_NAX_QKV");
    if (!candidate) {
    ...
    if (!h3_gpu_require_bf16(gpu, input, (size_t)rows * input_dim,
        ...
        !h3_gpu_require_command(gpu)) return 0;
    uint32_t headMajor = h3_runtime_getenv("H3_DISABLE_HEAD_MAJOR_SDPA") == NULL;
    typedef struct {
...
int h3_gpu_grouped_qkv_linear_rope_int8(
    ...
    BOOL fused_rope = !use_slower_unfused_qkv_rope &&
        h3_runtime_getenv("H3_DISABLE_FUSED_INT8_QKV_ROPE") == NULL;
    BOOL local_scales = fused_rope && rows > 2048 &&
        !use_slower_uncached_int8_scales &&
        h3_runtime_getenv("H3_DISABLE_QKV_LOCAL_SCALES") == NULL;
    BOOL full_k = fused_rope && input_dim == 5376u &&
        h3_runtime_getenv("H3_DISABLE_QKV_FULL_K") == NULL;
    id<MTLComputePipelineState> projection = h3_gpu_pipeline(
    ...
    if (fused_rope && rows <= 2048 && !use_slower_scalar_qkv_rms &&
        h3_runtime_getenv("H3_DISABLE_VECTOR_QKV_RMS") == NULL)
        rms_mode |= 2u;
    if (fused_rope && h3_runtime_getenv("H3_DISABLE_VECTOR_QKV_ROPE") == NULL)
        rms_mode |= 4u;
    qkv_project_rope_args args = {
...
int h3_gpu_gqa_causal_bf16(h3_gpu *opaque, h3_gpu_tensor *output,
    ...
    if (!sequence || !query_heads || !kv_heads || !head_dim ||
        ...
        !h3_gpu_require_command(gpu)) return 0;
    if (h3_runtime_getenv("H3_MPS_GQA") && h3_gpu_gqa_mps(
...
int h3_gpu_pack_coreml_f16(h3_gpu *opaque, h3_gpu_tensor *output,
                           const h3_gpu_tensor *input, uint32_t elements) {
    H3GPU *gpu = GPU(opaque);
    if (!h3_gpu_require_bf16(gpu, input, elements, @"Core ML pack input") ||
        !h3_gpu_require_elements(gpu, output, elements,
                                 @"Core ML pack output")) return 0;
    if (TENSOR(output).dtype != H3_GPU_F16) {
        h3_gpu_set_error(gpu, @"Core ML pack output is not F16");
        return 0;
    }
    return h3_gpu_dispatch_1d(gpu, @"h3_pack_coreml_f16", elements,
        ^(id<MTLComputeCommandEncoder> encoder) {
            [encoder setBuffer:TENSOR(input).buffer offset:0 atIndex:0];
            [encoder setBuffer:TENSOR(output).buffer offset:0 atIndex:1];
            [encoder setBytes:&elements length:sizeof(elements) atIndex:2];
...
int h3_gpu_pack_ane_input_bf16(h3_gpu *opaque, h3_gpu_tensor *plane,
    ...
    return h3_gpu_pack_ane_input_bf16_rows(
        opaque, plane, input, 0, rows, input_dim, base, chunk_dim,
        plane_rows);
}

int h3_gpu_pack_ane_input_bf16_rows(h3_gpu *opaque, h3_gpu_tensor *plane,
                                    const h3_gpu_tensor *input,
                                    ...
                                    uint32_t plane_rows) {
    H3GPU *gpu = GPU(opaque);
    if (input_row_offset > UINT32_MAX - rows ||
        !h3_gpu_require_bf16(
            gpu, input, (size_t)(input_row_offset + rows) * input_dim,
                             @"ANE pack input") ||
        !h3_gpu_require_elements(gpu, plane,
                                 (size_t)plane_rows * chunk_dim,
                                 @"ANE pack plane")) return 0;
    struct {
        uint32_t rows, input_dim, base, chunk_dim, plane_rows;
        uint32_t input_row_offset;
    } args = {rows, input_dim, base, chunk_dim, plane_rows,
              input_row_offset};
    return h3_gpu_dispatch_2d(gpu, @"h3_ane_pack_bf16",
        plane_rows, chunk_dim,
        ^(id<MTLComputeCommandEncoder> encoder) {
            [encoder setBuffer:TENSOR(input).buffer offset:0 atIndex:0];
...
int h3_gpu_unpack_ane_output_bf16(h3_gpu *opaque, h3_gpu_tensor *output,
                                  ...
                                  float scale) {
    H3GPU *gpu = GPU(opaque);
    if (!h3_gpu_require_elements(gpu, plane,
                                 (size_t)plane_rows * output_dim,
                                 @"ANE unpack plane") ||
        !h3_gpu_require_bf16(gpu, output, (size_t)rows * output_dim,
                             @"ANE unpack output")) return 0;
    struct {
        uint32_t rows, output_dim, plane_rows;
        float scale;
    } args = {rows, output_dim, plane_rows, scale};
...
int h3_gpu_unpack_ane_output_bf16_rows_checked(
    ...
    H3GPU *gpu = GPU(opaque);
    if (output_row_offset > UINT32_MAX - rows ||
        !isfinite(scale) || !(scale > 0.0f) ||
        !h3_gpu_require_elements(gpu, plane,
                                 (size_t)plane_rows * output_dim,
                                 @"ANE row-split unpack plane") ||
        !h3_gpu_require_bf16(
            gpu, output,
            (size_t)(output_row_offset + rows) * output_dim,
            @"ANE row-split unpack output") ||
        !h3_gpu_require_elements(gpu, nonfinite_flag, 1,
                                 @"ANE row-split nonfinite flag")) return 0;
    if (TENSOR(plane).dtype != H3_GPU_F32 ||
        TENSOR(nonfinite_flag).dtype != H3_GPU_F32) {
        h3_gpu_set_error(gpu, @"ANE row-split staging tensors have "
                              "invalid dtypes");
        return 0;
    }
    struct {
        uint32_t rows, output_dim, plane_rows;
        float scale;
        uint32_t output_row_offset, block_index;
    } args = {rows, output_dim, plane_rows, scale,
              output_row_offset, block_index};
    return h3_gpu_dispatch_2d(gpu, @"h3_ane_unpack_bf16_rows_checked",
        rows, output_dim,
        ^(id<MTLComputeCommandEncoder> encoder) {
            [encoder setBuffer:TENSOR(plane).buffer offset:0 atIndex:0];
            [encoder setBuffer:TENSOR(output).buffer offset:0 atIndex:1];
            [encoder setBuffer:TENSOR(nonfinite_flag).buffer offset:0
                        atIndex:2];
            [encoder setBytes:&args length:sizeof(args) atIndex:3];
        });
}
...
int h3_gpu_unpack_ane_output_bf16_rows_checked_range(
    ...
    if (output_row_offset > UINT32_MAX - rows ||
        ...
        !h3_gpu_require_elements(gpu, plane,
                                 (size_t)plane_rows * output_dim,
                                 @"ANE row-split range plane") ||
        !h3_gpu_require_bf16(
            gpu, output,
            (size_t)(output_row_offset + rows) * output_dim,
            @"ANE row-split range output") ||
        !h3_gpu_require_elements(gpu, range_stats, 2,
                                 @"ANE row-split range stats")) return 0;
    if (TENSOR(plane).dtype != H3_GPU_F32 ||
        TENSOR(range_stats).dtype != H3_GPU_F32) {
        h3_gpu_set_error(gpu, @"ANE row-split range tensors have invalid "
                              "dtypes");
        return 0;
    }
    if (!h3_gpu_require_command(gpu)) return 0;
    id<MTLComputePipelineState> pipeline = h3_gpu_pipeline(
        gpu, @"h3_ane_unpack_bf16_rows_checked_range");
    if (!pipeline || pipeline.maxTotalThreadsPerThreadgroup < 256) {
        h3_gpu_set_error(gpu,
                         @"ANE row-split range kernel requires 256 threads");
        return 0;
    }
    struct {
        uint32_t rows, output_dim, plane_rows;
    ...
    @autoreleasepool {
        ...
        [encoder setBuffer:TENSOR(plane).buffer offset:0 atIndex:0];
        [encoder setBuffer:TENSOR(output).buffer offset:0 atIndex:1];
        [encoder setBuffer:TENSOR(range_stats).buffer offset:0 atIndex:2];
        [encoder setBytes:&args length:sizeof(args) atIndex:3];
        [encoder dispatchThreadgroups:
            MTLSizeMake((rows + 15u) / 16u,
                        (output_dim + 15u) / 16u, 1)
                 threadsPerThreadgroup:MTLSizeMake(16, 16, 1)];
        [encoder endEncoding];
    }
    h3_gpu_stats stats = gpu.stats;
...
int h3_gpu_join_ane_linear_output_bf16_checked(
    ...
    H3GPU *gpu = GPU(opaque);
    uint32_t output_width = ane_width + gpu_width;
    if (!rows || !ane_width || !gpu_width || output_width < ane_width ||
        plane_rows < rows ||
        !h3_gpu_require_elements(gpu, ane_prefix,
                                 (size_t)plane_rows * ane_width,
                                 @"ANE linear prefix") ||
        !h3_gpu_require_bf16(gpu, gpu_suffix,
                             (size_t)rows * gpu_width,
                             @"GPU linear suffix") ||
        !h3_gpu_require_bf16(gpu, output,
                             (size_t)rows * output_width,
                             @"heterogeneous linear output") ||
        !h3_gpu_require_elements(gpu, nonfinite_flag, 1,
                                 @"ANE linear nonfinite flag")) return 0;
    if (TENSOR(nonfinite_flag).dtype != H3_GPU_F32) {
        h3_gpu_set_error(gpu, @"ANE linear nonfinite flag is not F32");
        return 0;
    }
    struct {
        uint32_t rows, ane_width, gpu_width, plane_rows, block_index;
    } args = {rows, ane_width, gpu_width, plane_rows, block_index};
    int prefix_ok = h3_gpu_dispatch_2d(
        gpu, @"h3_join_ane_linear_prefix_bf16_checked",
        rows, ane_width,
        ^(id<MTLComputeCommandEncoder> encoder) {
            [encoder setBuffer:TENSOR(ane_prefix).buffer offset:0 atIndex:0];
            [encoder setBuffer:TENSOR(output).buffer offset:0 atIndex:2];
            [encoder setBuffer:TENSOR(nonfinite_flag).buffer
                         offset:0 atIndex:3];
            [encoder setBytes:&args length:sizeof(args) atIndex:4];
    ...
    return h3_gpu_dispatch_2d(
        gpu, @"h3_join_gpu_linear_suffix_bf16",
        gpu_width, rows,
        ^(id<MTLComputeCommandEncoder> encoder) {
            [encoder setBuffer:TENSOR(gpu_suffix).buffer offset:0 atIndex:1];
            [encoder setBuffer:TENSOR(output).buffer offset:0 atIndex:2];
            [encoder setBytes:&args length:sizeof(args) atIndex:4];
        });
}

int h3_gpu_pack_coreml_f16_transpose(h3_gpu *opaque,
                                     h3_gpu_tensor *output,
                                     const h3_gpu_tensor *input,
    ...
    if (!rows || !hidden ||
        !h3_gpu_require_bf16(gpu, input, elements,
                             @"Core ML transpose pack input") ||
        !h3_gpu_require_elements(gpu, output, elements,
                                 @"Core ML transpose pack output")) return 0;
    if (TENSOR(output).dtype != H3_GPU_F16) {
        h3_gpu_set_error(gpu, @"Core ML transpose pack output is not F16");
        return 0;
    }
    if (!h3_gpu_require_command(gpu)) return 0;
    id<MTLComputePipelineState> pipeline = h3_gpu_pipeline(
        gpu, @"h3_pack_coreml_f16_transpose");
    if (!pipeline || pipeline.maxTotalThreadsPerThreadgroup < 256) return 0;
    struct { uint32_t rows, hidden; } args = {rows, hidden};
    @autoreleasepool {
        id<MTLComputeCommandEncoder> encoder =
            [gpu.command computeCommandEncoder];
        [encoder setComputePipelineState:pipeline];
        ...
        [encoder setBytes:&args length:sizeof(args) atIndex:2];
        [encoder dispatchThreadgroups:
            MTLSizeMake((hidden + 15u) / 16u, (rows + 15u) / 16u, 1)
                 threadsPerThreadgroup:MTLSizeMake(16, 16, 1)];
        [encoder endEncoding];
    }
    h3_gpu_stats stats = gpu.stats;
...
int h3_gpu_add_coreml_f16_bf16(h3_gpu *opaque, h3_gpu_tensor *output,
                               const h3_gpu_tensor *gpu_partial,
                               const h3_gpu_tensor *ane_partial,
                               uint32_t elements) {
    H3GPU *gpu = GPU(opaque);
    if (!h3_gpu_require_bf16(gpu, gpu_partial, elements,
                             @"Core ML join GPU partial") ||
        !h3_gpu_require_elements(gpu, ane_partial, elements,
                                 @"Core ML join ANE partial") ||
        !h3_gpu_require_bf16(gpu, output, elements,
                             @"Core ML join output")) return 0;
    if (TENSOR(ane_partial).dtype != H3_GPU_F16) {
        h3_gpu_set_error(gpu, @"Core ML join ANE partial is not F16");
        return 0;
    }
    return h3_gpu_dispatch_1d(gpu, @"h3_add_coreml_f16_bf16", elements,
        ^(id<MTLComputeCommandEncoder> encoder) {
            [encoder setBuffer:TENSOR(gpu_partial).buffer offset:0 atIndex:0];
            [encoder setBuffer:TENSOR(ane_partial).buffer offset:0 atIndex:1];
            [encoder setBuffer:TENSOR(output).buffer offset:0 atIndex:2];
            [encoder setBytes:&elements length:sizeof(elements) atIndex:3];
        });
}
...
int h3_gpu_add_coreml_f16_bf16_transpose(
    ...
    size_t elements = (size_t)rows * hidden;
    if (!rows || !hidden ||
        !h3_gpu_require_bf16(gpu, gpu_partial, elements,
                             @"Core ML transpose join GPU partial") ||
        !h3_gpu_require_elements(gpu, ane_partial, elements,
                                 @"Core ML transpose join ANE partial") ||
        !h3_gpu_require_bf16(gpu, output, elements,
                             @"Core ML transpose join output")) return 0;
    if (TENSOR(ane_partial).dtype != H3_GPU_F16) {
        h3_gpu_set_error(gpu,
                         @"Core ML transpose join ANE partial is not F16");
        return 0;
    }
    if (!h3_gpu_require_command(gpu)) return 0;
    id<MTLComputePipelineState> pipeline = h3_gpu_pipeline(
        gpu, @"h3_add_coreml_f16_bf16_transpose");
    if (!pipeline || pipeline.maxTotalThreadsPerThreadgroup < 256) return 0;
    struct {
        uint32_t rows, hidden;
        float ane_scale;
    } args = {rows, hidden, ane_scale};
    @autoreleasepool {
        id<MTLComputeCommandEncoder> encoder =
            [gpu.command computeCommandEncoder];
        [encoder setComputePipelineState:pipeline];
...
int h3_gpu_add_coreml_f16_bf16_transpose_checked(
    ...
    H3GPU *gpu = GPU(opaque);
    size_t elements = (size_t)rows * hidden;
    if (!rows || !hidden || !isfinite(ane_scale) || !(ane_scale > 0.0f) ||
        !h3_gpu_require_bf16(gpu, gpu_partial, elements,
                             @"checked Core ML join GPU partial") ||
        !h3_gpu_require_elements(gpu, ane_partial, elements,
                                 @"checked Core ML join ANE partial") ||
        !h3_gpu_require_bf16(gpu, output, elements,
                             @"checked Core ML join output") ||
        !h3_gpu_require_elements(gpu, nonfinite_flag, 1,
                                 @"checked Core ML nonfinite flag")) return 0;
    if (TENSOR(ane_partial).dtype != H3_GPU_F16 ||
        TENSOR(nonfinite_flag).dtype != H3_GPU_F32) {
        h3_gpu_set_error(gpu,
                         @"checked Core ML join requires F16 ANE partial "
                          "and F32 flag");
        return 0;
    }
    if (!h3_gpu_require_command(gpu)) return 0;
    id<MTLComputePipelineState> pipeline = h3_gpu_pipeline(
        gpu, @"h3_add_coreml_f16_bf16_transpose_checked");
    if (!pipeline || pipeline.maxTotalThreadsPerThreadgroup < 256) return 0;
    struct {
        uint32_t rows, hidden;
        float ane_scale;
        uint32_t block_index;
    } args = {rows, hidden, ane_scale, block_index};
    @autoreleasepool {
        id<MTLComputeCommandEncoder> encoder =
            [gpu.command computeCommandEncoder];
        [encoder setComputePipelineState:pipeline];
        ...
        [encoder setBuffer:TENSOR(ane_partial).buffer offset:0 atIndex:1];
        [encoder setBuffer:TENSOR(output).buffer offset:0 atIndex:2];
        [encoder setBuffer:TENSOR(nonfinite_flag).buffer offset:0 atIndex:3];
        [encoder setBytes:&args length:sizeof(args) atIndex:4];
        [encoder dispatchThreadgroups:
            MTLSizeMake((hidden + 15u) / 16u, (rows + 15u) / 16u, 1)
                 threadsPerThreadgroup:MTLSizeMake(16, 16, 1)];
...
int h3_gpu_add_ane_f32_bf16_transpose_checked(
                               ...
                               uint32_t block_index) {
    H3GPU *gpu = GPU(opaque);
    if (!h3_gpu_require_bf16(gpu, gpu_partial, (size_t)rows * hidden,
                             @"private ANE GPU partial") ||
        !h3_gpu_require_elements(gpu, ane_partial,
                                 (size_t)plane_rows * hidden,
                                 @"private ANE partial") ||
        !h3_gpu_require_bf16(gpu, output, (size_t)rows * hidden,
                             @"private ANE joined output") ||
        !h3_gpu_require_elements(gpu, nonfinite_flag, 1,
                                 @"private ANE nonfinite flag")) return 0;
    struct {
        uint32_t rows, hidden, plane_rows;
        float ane_scale;
        uint32_t block_index;
    } args = {rows, hidden, plane_rows, ane_scale, block_index};
    return h3_gpu_dispatch_2d(gpu,
        @"h3_add_ane_f32_bf16_transpose_checked", rows, hidden,
        ^(id<MTLComputeCommandEncoder> encoder) {
            [encoder setBuffer:TENSOR(gpu_partial).buffer offset:0 atIndex:0];
            [encoder setBuffer:TENSOR(ane_partial).buffer offset:0 atIndex:1];
            [encoder setBuffer:TENSOR(output).buffer offset:0 atIndex:2];
...
int h3_gpu_add_ane_f32_bf16_transpose_gate_adaln_checked(
    ...
    H3GPU *gpu = GPU(opaque);
    size_t elements = (size_t)rows * hidden;
    if (!rows || !hidden || hidden > 5376 || gate_slot >= slots ||
        shift_slot >= slots || scale_slot >= slots ||
        !isfinite(ane_scale) || !(ane_scale > 0.0f) ||
        !isfinite(epsilon) || !(epsilon > 0.0f) ||
        !h3_gpu_require_bf16(gpu, gpu_partial, elements,
                             @"fused private ANE GPU partial") ||
        !h3_gpu_require_elements(gpu, ane_partial,
                                 (size_t)plane_rows * hidden,
                                 @"fused private ANE partial") ||
        !h3_gpu_require_elements(gpu, nonfinite_flag, 1,
                                 @"fused private ANE nonfinite flag") ||
        !h3_gpu_require_bf16(gpu, residual, elements,
                             @"fused private ANE residual") ||
        !h3_gpu_require_bf16(gpu, norm_weight, hidden,
                             @"fused private ANE next norm") ||
        !h3_gpu_require_bf16(gpu, gate_modulation, 1,
                             @"fused private ANE gate modulation") ||
        !h3_gpu_require_bf16(gpu, norm_modulation, 1,
                             @"fused private ANE norm modulation") ||
        !h3_gpu_require_elements(gpu, row_map, rows,
                                 @"fused private ANE row map") ||
        !h3_gpu_require_bf16(gpu, branch_output, elements,
                             @"fused private ANE branch output") ||
        !h3_gpu_require_bf16(gpu, gated_residual, elements,
                             @"fused private ANE gated residual") ||
        !h3_gpu_require_bf16(gpu, norm_output, elements,
                             @"fused private ANE norm output") ||
        TENSOR(ane_partial).dtype != H3_GPU_F32 ||
        TENSOR(nonfinite_flag).dtype != H3_GPU_F32 ||
        TENSOR(row_map).dtype != H3_GPU_U32 ||
        !h3_gpu_require_command(gpu)) return 0;
    typedef struct {
        uint32_t rows, hidden, plane_rows, slots;
        uint32_t gate_slot, shift_slot, scale_slot;
        float ane_scale, epsilon;
        uint32_t block_index;
    } fused_args;
    fused_args args = {
        rows, hidden, plane_rows, slots,
        gate_slot, shift_slot, scale_slot,
        ane_scale, epsilon, block_index
    };
    id<MTLComputePipelineState> pipeline = h3_gpu_pipeline(
        gpu, @"h3_add_ane_f32_bf16_transpose_gate_adaln_checked");
    const NSUInteger threads = 256;
    if (!pipeline || pipeline.maxTotalThreadsPerThreadgroup < threads) {
        h3_gpu_set_error(gpu,
            @"fused private ANE join/gate/AdaLN needs a 256-thread "
             "threadgroup");
        return 0;
    }
    @autoreleasepool {
        ...
        [encoder setBuffer:TENSOR(gpu_partial).buffer offset:0 atIndex:0];
        [encoder setBuffer:TENSOR(ane_partial).buffer offset:0 atIndex:1];
        [encoder setBuffer:TENSOR(branch_output).buffer offset:0 atIndex:2];
        [encoder setBuffer:TENSOR(nonfinite_flag).buffer offset:0 atIndex:3];
        [encoder setBuffer:TENSOR(residual).buffer offset:0 atIndex:4];
        [encoder setBuffer:TENSOR(gate_modulation).buffer offset:0 atIndex:5];
        [encoder setBuffer:TENSOR(row_map).buffer offset:0 atIndex:6];
        [encoder setBuffer:TENSOR(norm_weight).buffer offset:0 atIndex:7];
        [encoder setBuffer:TENSOR(norm_modulation).buffer offset:0 atIndex:8];
        [encoder setBuffer:TENSOR(gated_residual).buffer offset:0 atIndex:9];
        [encoder setBuffer:TENSOR(norm_output).buffer offset:0 atIndex:10];
        [encoder setBytes:&args length:sizeof(args) atIndex:11];
        [encoder dispatchThreadgroups:MTLSizeMake(rows, 1, 1)
                 threadsPerThreadgroup:MTLSizeMake(threads, 1, 1)];
        [encoder endEncoding];
...
int h3_gpu_add_ane_f32_bf16_transpose_gate_adaln_quantize_int8_checked(
    ...
    H3GPU *gpu = GPU(opaque);
    size_t elements = (size_t)rows * hidden;
    size_t padded_elements = (size_t)padded_rows * hidden;
    if (!gpu.tensorOpsEnabled || !rows || padded_rows < rows || !hidden ||
        hidden > 5376 || plane_rows < rows || gate_slot >= slots ||
        shift_slot >= slots || scale_slot >= slots ||
        !isfinite(ane_scale) || !(ane_scale > 0.0f) ||
        !isfinite(epsilon) || !(epsilon > 0.0f) ||
        !h3_gpu_require_bf16(gpu, gpu_partial, elements,
                             @"fused int8 private ANE GPU partial") ||
        !h3_gpu_require_elements(gpu, ane_partial,
                                 (size_t)plane_rows * hidden,
                                 @"fused int8 private ANE partial") ||
        !h3_gpu_require_elements(gpu, nonfinite_flag, 1,
                                 @"fused int8 private ANE nonfinite flag") ||
        !h3_gpu_require_bf16(gpu, residual, elements,
                             @"fused int8 private ANE residual") ||
        !h3_gpu_require_bf16(gpu, norm_weight, hidden,
                             @"fused int8 private ANE next norm") ||
        !h3_gpu_require_bf16(gpu, gate_modulation, 1,
                             @"fused int8 private ANE gate modulation") ||
        !h3_gpu_require_bf16(gpu, norm_modulation, 1,
                             @"fused int8 private ANE norm modulation") ||
        !h3_gpu_require_elements(gpu, row_map, rows,
                                 @"fused int8 private ANE row map") ||
        !h3_gpu_require_bf16(gpu, branch_output, elements,
                             @"fused int8 private ANE branch output") ||
        !h3_gpu_require_bf16(gpu, gated_residual, elements,
                             @"fused int8 private ANE gated residual") ||
        !h3_gpu_require_i8(gpu, quantized_output, padded_elements,
                           @"fused int8 private ANE AdaLN output") ||
        !h3_gpu_require_f32(gpu, quantized_scales, padded_rows,
                            @"fused int8 private ANE AdaLN scales") ||
        TENSOR(ane_partial).dtype != H3_GPU_F32 ||
        TENSOR(nonfinite_flag).dtype != H3_GPU_F32 ||
        TENSOR(row_map).dtype != H3_GPU_U32 ||
        !h3_gpu_require_command(gpu)) return 0;
    ...
    NSString *pipeline_name = hidden == 5376 &&
        h3_runtime_getenv("H3_DISABLE_VECTOR_GATE_ADALN") == NULL ?
        @"h3_add_ane_f32_bf16_transpose_gate_adaln_quantize_int8_checked" :
        @"h3_add_ane_f32_bf16_transpose_gate_adaln_quantize_int8_checked_scalar";
    id<MTLComputePipelineState> pipeline = h3_gpu_pipeline(
    ...
    if (!pipeline || pipeline.maxTotalThreadsPerThreadgroup < threads) {
        h3_gpu_set_error(gpu,
            @"fused private ANE join/gate/AdaLN/int8 needs a 256-thread "
             "threadgroup");
        return 0;
    }
    @autoreleasepool {
        ...
        [encoder setBuffer:TENSOR(norm_modulation).buffer offset:0 atIndex:8];
        [encoder setBuffer:TENSOR(gated_residual).buffer offset:0 atIndex:9];
        [encoder setBuffer:TENSOR(quantized_output).buffer offset:0 atIndex:10];
        [encoder setBuffer:TENSOR(quantized_scales).buffer offset:0 atIndex:11];
...
enum { H3_RELATIVE_L1_GROUP_ELEMENTS = 2048 };

uint32_t h3_gpu_relative_l1_partial_count(uint32_t elements) {
    return elements ?
        (elements + H3_RELATIVE_L1_GROUP_ELEMENTS - 1u) /
            H3_RELATIVE_L1_GROUP_ELEMENTS : 0u;
}

int h3_gpu_relative_l1_residual_bf16(
                    h3_gpu *opaque, h3_gpu_tensor *partials,
                    const h3_gpu_tensor *head_output,
                    const h3_gpu_tensor *head_input,
                    const h3_gpu_tensor *previous_residual,
                    uint32_t elements) {
    H3GPU *gpu = GPU(opaque);
    uint32_t groups = h3_gpu_relative_l1_partial_count(elements);
    size_t partial_elements = (size_t)groups * 2;
    if (!groups || !h3_gpu_require_command(gpu) ||
        !h3_gpu_require_bf16(gpu, head_output, elements,
                             @"relative-L1 head output") ||
        !h3_gpu_require_bf16(gpu, head_input, elements,
                             @"relative-L1 head input") ||
        !h3_gpu_require_bf16(gpu, previous_residual, elements,
                             @"relative-L1 previous residual") ||
        !h3_gpu_require_elements(gpu, partials, partial_elements,
                                 @"relative-L1 partials")) return 0;
    if (TENSOR(partials).dtype != H3_GPU_F32) {
        h3_gpu_set_error(gpu, @"relative-L1 partial tensor must be F32");
        return 0;
    }
    id<MTLComputePipelineState> pipeline = h3_gpu_pipeline(
        gpu, @"h3_relative_l1_residual_bf16");
    if (!pipeline) return 0;
    NSUInteger threads = MIN((NSUInteger)256,
                             pipeline.maxTotalThreadsPerThreadgroup);
    if (threads != 256) {
        h3_gpu_set_error(gpu,
                         @"relative-L1 kernel requires 256 threads");
        return 0;
    }
    @autoreleasepool {
        id<MTLComputeCommandEncoder> encoder =
            [gpu.command computeCommandEncoder];
        [encoder setComputePipelineState:pipeline];
        [encoder setBuffer:TENSOR(head_output).buffer offset:0 atIndex:0];
        [encoder setBuffer:TENSOR(head_input).buffer offset:0 atIndex:1];
        [encoder setBuffer:TENSOR(previous_residual).buffer offset:0 atIndex:2];
...
int h3_gpu_relative_l1_bf16_range(
    ...
    H3GPU *gpu = GPU(opaque);
    uint32_t groups = h3_gpu_relative_l1_partial_count(elements);
    size_t input_elements = (size_t)element_offset + elements;
    size_t partial_groups = (size_t)partial_group_offset + groups;
    if (input_elements > UINT32_MAX || partial_groups > SIZE_MAX / 2) {
        h3_gpu_set_error(gpu, @"relative-L1 span exceeds tensor limits");
        return 0;
    }
    size_t partial_elements = partial_groups * 2;
    if (!groups || !h3_gpu_require_command(gpu) ||
        !h3_gpu_require_bf16(gpu, current, input_elements,
                             @"relative-L1 current input") ||
        !h3_gpu_require_bf16(gpu, previous, input_elements,
                             @"relative-L1 previous input") ||
        !h3_gpu_require_elements(gpu, partials, partial_elements,
                                 @"relative-L1 partials")) return 0;
    if (TENSOR(partials).dtype != H3_GPU_F32) {
        h3_gpu_set_error(gpu, @"relative-L1 partial tensor must be F32");
    ...
    @autoreleasepool {
        ...
        [encoder setBuffer:TENSOR(previous).buffer offset:0 atIndex:1];
        [encoder setBuffer:TENSOR(partials).buffer offset:0 atIndex:2];
        [encoder setBytes:&elements length:sizeof(elements) atIndex:3];
        [encoder setBytes:&element_offset length:sizeof(element_offset)
                  atIndex:4];
        [encoder setBytes:&partial_group_offset
                   length:sizeof(partial_group_offset) atIndex:5];
        [encoder dispatchThreadgroups:MTLSizeMake(groups, 1, 1)
                 threadsPerThreadgroup:MTLSizeMake(threads, 1, 1)];
        [encoder endEncoding];
    }
...
int h3_gpu_relative_l1_bf16(h3_gpu *opaque, h3_gpu_tensor *partials,
                            const h3_gpu_tensor *current,
                            const h3_gpu_tensor *previous,
                            uint32_t elements) {
    return h3_gpu_relative_l1_bf16_range(
        opaque, partials, 0, current, previous, 0, elements);
}

uint32_t h3_gpu_compare_bf16_partial_count(uint32_t elements) {
    return h3_gpu_relative_l1_partial_count(elements);
}

int h3_gpu_compare_bf16(h3_gpu *opaque, h3_gpu_tensor *partials,
                        const h3_gpu_tensor *candidate,
                        const h3_gpu_tensor *reference,
                        uint32_t elements) {
    H3GPU *gpu = GPU(opaque);
    uint32_t groups = h3_gpu_compare_bf16_partial_count(elements);
    size_t partial_elements = (size_t)groups * 4;
    if (!groups || !h3_gpu_require_command(gpu) ||
        !h3_gpu_require_bf16(gpu, candidate, elements,
                             @"BF16 comparison candidate") ||
        !h3_gpu_require_bf16(gpu, reference, elements,
                             @"BF16 comparison reference") ||
        !h3_gpu_require_elements(gpu, partials, partial_elements,
                                 @"BF16 comparison partials")) return 0;
    if (TENSOR(partials).dtype != H3_GPU_F32) {
        h3_gpu_set_error(gpu, @"BF16 comparison partial tensor must be F32");
        return 0;
    }
    id<MTLComputePipelineState> pipeline = h3_gpu_pipeline(
        gpu, @"h3_compare_bf16");
    ...
    NSUInteger threads = MIN((NSUInteger)256,
                             pipeline.maxTotalThreadsPerThreadgroup);
    if (threads != 256) {
        h3_gpu_set_error(gpu, @"BF16 comparison kernel requires 256 threads");
        return 0;
    }
    @autoreleasepool {
        id<MTLComputeCommandEncoder> encoder =
            [gpu.command computeCommandEncoder];
        [encoder setComputePipelineState:pipeline];
        [encoder setBuffer:TENSOR(candidate).buffer offset:0 atIndex:0];
        [encoder setBuffer:TENSOR(reference).buffer offset:0 atIndex:1];
        [encoder setBuffer:TENSOR(partials).buffer offset:0 atIndex:2];
        [encoder setBytes:&elements length:sizeof(elements) atIndex:3];
        [encoder dispatchThreadgroups:MTLSizeMake(groups, 1, 1)
                 threadsPerThreadgroup:MTLSizeMake(threads, 1, 1)];
...
int h3_gpu_compare_ane_f32_bf16_prefix(
    ...
    H3GPU *gpu = GPU(opaque);
    size_t logical_elements = (size_t)rows * candidate_width;
    size_t candidate_elements = (size_t)plane_rows * candidate_width;
    size_t reference_elements = (size_t)rows * reference_width;
    if (!rows || !candidate_width || candidate_width > reference_width ||
        rows > plane_rows || logical_elements > UINT32_MAX) {
        h3_gpu_set_error(gpu, @"private ANE comparison shape is invalid");
        return 0;
    }
    uint32_t elements = (uint32_t)logical_elements;
    uint32_t groups =
        h3_gpu_compare_ane_f32_bf16_partial_count(elements);
    size_t partial_elements = (size_t)groups * 8;
    if (!groups || !h3_gpu_require_command(gpu) ||
        !h3_gpu_require_elements(gpu, candidate, candidate_elements,
                                 @"private ANE comparison candidate") ||
        !h3_gpu_require_bf16(gpu, reference, reference_elements,
                             @"private ANE comparison reference") ||
        !h3_gpu_require_elements(gpu, partials, partial_elements,
                                 @"private ANE comparison partials")) return 0;
    if (TENSOR(candidate).dtype != H3_GPU_F32 ||
        TENSOR(partials).dtype != H3_GPU_F32) {
        h3_gpu_set_error(
            gpu, @"private ANE comparison candidate/partials must be F32");
        return 0;
    }
    id<MTLComputePipelineState> pipeline = h3_gpu_pipeline(
        gpu, @"h3_compare_ane_f32_bf16_prefix");
    if (!pipeline) return 0;
    NSUInteger threads = MIN((NSUInteger)256,
                             pipeline.maxTotalThreadsPerThreadgroup);
    if (threads != 256) {
        h3_gpu_set_error(
            gpu, @"private ANE comparison kernel requires 256 threads");
        return 0;
    }
    typedef struct {
        uint32_t rows;
    ...
    h3_ane_compare_args args = {
        rows, candidate_width, reference_width, plane_rows, elements
    };
    @autoreleasepool {
        id<MTLComputeCommandEncoder> encoder =
            [gpu.command computeCommandEncoder];
