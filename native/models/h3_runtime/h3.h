...
typedef struct {
    ...
    int frames;
    int steps;
    /* Flow-matching time shifts. Released H3 and LightX2V v0.1 use 12/3;
     * the provenance-pinned LightX2V v1.1 768p adapter uses 6/3. */
    double video_flow_shift;
    double audio_flow_shift;
    uint64_t seed;
    ...
    int preview_denoise;
    /* Transfer ownership of the final decoded F32 RGB frames and PCM into the
