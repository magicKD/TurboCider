...
static NSURL *compiled_model_url(NSURL *source, BOOL *used_cache,
    ...
    if (h3_runtime_getenv("H3_COREML_USE_COMPILED")) {
...
static dispatch_semaphore_t coreml_load_slots(void) {
    ...
    dispatch_once(&once, ^{
        long jobs = 4;
        const char *value = h3_runtime_getenv("H3_COREML_LOAD_JOBS");
        if (value && *value) {
            char *tail = NULL;
...
static NSString *load_coreml_object(H3CoreMLMLP *object, NSURL *source,
    ...
    BOOL profile = h3_runtime_getenv("H3_PROFILE") != NULL;
    ...
    hints.reshapeFrequency = h3_runtime_getenv("H3_COREML_RESHAPE_FREQUENT") ?
        MLReshapeFrequencyHintFrequent :
        MLReshapeFrequencyHintInfrequent;
    hints.specializationStrategy = h3_runtime_getenv("H3_COREML_SPECIALIZE_FAST") ?
