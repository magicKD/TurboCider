...
void ltx_st_free_header(ltx_st_header *header) {
    ...
    free(header->metadata_gemma_config);
    free(header->path);
            ...
            if (!tensors && tensor_count) {
                ...
                if (ok) {
                    ...
                    NSString *config = [metadata isKindOfClass:[NSDictionary class]] ?
                        metadata[@"config"] : nil;
                    NSString *gemma_config =
                        [metadata isKindOfClass:[NSDictionary class]] ?
                        metadata[@"gemma_config"] : nil;
                    ...
                    if ([gemma_config isKindOfClass:[NSString class]])
                        header->metadata_gemma_config =
                            strdup(gemma_config.UTF8String);
                    if (!header->path ||
                        ([config isKindOfClass:[NSString class]] &&
                         !header->metadata_config) ||
                        ([gemma_config isKindOfClass:[NSString class]] &&
                         !header->metadata_gemma_config)) {
                        ok = ltx_st_fail(error, error_size,
