...
struct FluxVaeOps {
    ...
    Tensor group_norm(const Tensor& a,const std::string& p) {
        ...
        auto z=mx::reshape(mx::transpose(mx::reshape(f,{1,32,n,c/32}),{0,2,1,3}),a.shape());
        return mx::astype(z*mx::astype(weights.at(p+".weight"),mx::float32)+mx::astype(weights.at(p+".bias"),mx::float32),mx::bfloat16);
    }
    ...
    Tensor attention(const Tensor& x,const std::string& p) {
        int n=x.shape(1)*x.shape(2),c=x.shape(3);auto z=group_norm(x,p+".group_norm");
