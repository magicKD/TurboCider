...
static void profile_keys(NSDictionary *d,NSArray *allowed) {
    ...
    for(NSString *key in d)require([keys containsObject:key],"unknown profile field: "+std::string(key.UTF8String));
}
...
void resolve_profile(Request& r) {
    ...
    auto path=std::filesystem::absolute(r.profile);auto d=read_json(path);
    ...
    auto bounded=[&](NSString *key,uint64_t fallback,uint64_t upper) {
        id value=model[key];if(!value)return fallback;
        require([value isKindOfClass:NSNumber.class]&&CFGetTypeID((__bridge CFTypeRef)value)!=CFBooleanGetTypeID(),"profile budget must be numeric");
        double x=[value doubleValue];uint64_t n=[value unsignedLongLongValue];require(x>=0&&x<=double(upper)&&double(n)==x,"invalid profile budget");return n;
    };
    r.memory_budget_bytes=bounded(@"memory_budget_bytes",0,physical);
    r.allocator_cache_bytes=bounded(@"allocator_cache_bytes",r.allocator_cache_bytes,physical/4);
    r.warmup_iterations=int(bounded(@"warmup_iterations",0,8));
    auto content=json(d);unsigned char digest[CC_SHA256_DIGEST_LENGTH];CC_SHA256(content.data(),CC_LONG(content.size()),digest);
    for(auto byte:digest){char pair[3];snprintf(pair,sizeof(pair),"%02x",byte);r.profile_identity+=pair;}
}
}
