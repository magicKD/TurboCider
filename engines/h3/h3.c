...
static char *h3_prepared_key(const char *conditioning,
                             ...
                             int render_width, int render_height) {
    h3_key key = {0};
    const char *layer_policy = getenv("H3_DIT_LAYER_POLICY");
    const char *gate_skip = getenv("H3_DIT_GATE_SKIP");
    const char *gate_cache = getenv("H3_DIT_GATE_CACHE");
...
static int h3_dump_video_latent(h3_ctx *ctx, const float *values,
                                ...
                                int width) {
    const char *path = getenv("H3_DUMP_VIDEO_LATENT");
...
static int h3_parallel_prepare_requested(void) {
    const char *value = getenv("H3_PARALLEL_PREPARE");
...
h3_result *h3_generate(h3_ctx *ctx, const char *prompt,
    ...
    const char *taeh3_weights = getenv("H3_TAEH3_WEIGHTS");
    ...
    } else if (h3_parallel_prepare_requested() && getenv("H3_PROFILE")) {
        fprintf(stderr,
