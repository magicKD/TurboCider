...
static int configure_first_block_cache(h3_dit *dit, char *error,
                                       size_t error_size) {
    const char *value = getenv("H3_FBC_THRESHOLD");
    ...
    unsigned max_hits = 1;
    const char *max_hits_text = getenv("H3_FBC_MAX_HITS");
...
static int configure_tea_cache(h3_dit *dit, char *error,
                               size_t error_size) {
    const char *value = getenv("H3_TEACACHE_THRESHOLD");
    if (!value || !*value) return 1;
    char *tail = NULL;
    float threshold = strtof(value, &tail);
    if (tail == value || *tail || !isfinite(threshold) ||
    ...
    const char *audio_value = getenv("H3_TEACACHE_AUDIO_THRESHOLD");
...
static int configure_sol_attention(h3_dit *dit, char *error,
    ...
    float tau = 1.0f;
    const char *tau_text = getenv("H3_SOL_TAU");
...
static int parse_coreml_block(unsigned *selected,
                              char *error, size_t error_size) {
    const char *model = getenv("H3_COREML_ANE_MODEL");
    if (!model || !*model) return 0;
    const char *value = getenv("H3_COREML_ANE_BLOCK");
    if (!value || !*value) {
        *selected = 0;
        return 1;
    }
...
static int coreml_mlp_enabled(unsigned index,
                              char *error, size_t error_size) {
    const char *directory = getenv("H3_COREML_ANE_DIR");
    const char *single_model = getenv("H3_COREML_ANE_MODEL");
    ...
    if (directory && *directory) {
        return coreml_block_in_list(
            "H3_COREML_ANE_BLOCKS", getenv("H3_COREML_ANE_BLOCKS"),
...
static int private_ane_mlp_enabled(void) {
    const char *directory = getenv("H3_PRIVATE_ANE_MLP_DIR");
...
static int configure_private_ane_mlp_row_split(h3_dit *dit,
                                                char *error,
                                                size_t error_size) {
    const char *value = getenv("H3_PRIVATE_ANE_MLP_ROW_SPLIT_ROWS");
    if (!value || !*value) return 1;
    char *tail = NULL;
    unsigned long parsed = strtoul(value, &tail, 10);
...
static int configure_private_ane_mlp_range_guard(h3_dit *dit,
    ...
    const char *headroom_value =
        getenv("H3_PRIVATE_ANE_MLP_RANGE_HEADROOM");
    ...
    const char *retry_value =
        getenv("H3_PRIVATE_ANE_MLP_RANGE_RETRIES");
...
static int private_ane_mlp_block_enabled(unsigned index,
    ...
    const char *blocks = getenv("H3_PRIVATE_ANE_MLP_BLOCKS");
...
static int private_ane_mlp_runtime_scale_for_block(
        ...
        char *error, size_t error_size) {
    *scale = 1.0f;
    const char *global = getenv("H3_PRIVATE_ANE_MLP_RUNTIME_SCALE");
    if (global && *global) {
        char *tail = NULL;
        errno = 0;
    ...
    const char *profile =
        getenv("H3_PRIVATE_ANE_MLP_RUNTIME_SCALE_PROFILE");
...
static int configure_private_ane_mlp(h3_dit *dit, h3_dit_block *block,
    ...
    if (getenv("H3_COREML_ANE_DIR") || getenv("H3_COREML_ANE_MODEL")) {
        fail(error, error_size,
    ...
    const char *precision = getenv("H3_PRIVATE_ANE_MLP_PRECISION");
...
static int coreml_qkv_enabled(unsigned index,
                              char *error, size_t error_size) {
    const char *directory = getenv("H3_COREML_QKV_DIR");
    if (!directory || !*directory) return 0;
    return coreml_block_in_list(
        "H3_COREML_QKV_BLOCKS", getenv("H3_COREML_QKV_BLOCKS"),
        index, error, error_size);
}

static int private_ane_qkv_enabled(unsigned index,
                                   char *error, size_t error_size) {
    const char *directory = getenv("H3_PRIVATE_ANE_QKV_DIR");
    const char *checkpoint = getenv("H3_PRIVATE_ANE_QKV_CHECKPOINT");
    if ((!directory || !*directory) &&
        (!checkpoint || !*checkpoint || !strcmp(checkpoint, "0"))) return 0;
    return coreml_block_in_list(
        "H3_PRIVATE_ANE_QKV_BLOCKS", getenv("H3_PRIVATE_ANE_QKV_BLOCKS"),
        index, error, error_size);
}

static int private_ane_qkv_micro_enabled(void) {
    const char *value = getenv("H3_PRIVATE_ANE_QKV_MICRO");
...
static int private_ane_qkv_micro_selected(int step,
    ...
    const char *value = getenv("H3_PRIVATE_ANE_QKV_MICRO_STEP");
    if (!value || !*value) return 1;
    char *tail = NULL;
...
static int configure_private_ane_qkv(h3_dit *dit, h3_dit_block *block,
    ...
    const char *checkpoint_value =
        getenv("H3_PRIVATE_ANE_QKV_CHECKPOINT");
    ...
    const char *heads_value = getenv("H3_PRIVATE_ANE_QKV_HEADS");
    ...
    const char *precision = getenv("H3_PRIVATE_ANE_QKV_PRECISION");
    int int8_weights = precision && *precision && !strcmp(precision, "int8");
    if (precision && *precision && !int8_weights &&
        strcmp(precision, "fp16")) {
        fail(error, error_size,
    ...
    const char *gpu_precision =
        getenv("H3_PRIVATE_ANE_QKV_GPU_PRECISION");
    ...
    if (checkpoint_source && !getenv("H3_BENCH_COREML_QKV_AB") && !micro)
...
static int configure_private_ane_attention_out(
    ...
    const char *width_value = getenv("H3_PRIVATE_ANE_ATTENTION_OUT_WIDTH");
    ...
    const char *precision =
        getenv("H3_PRIVATE_ANE_ATTENTION_OUT_PRECISION");
    int int8_weights = precision && *precision && !strcmp(precision, "int8");
    if (precision && *precision && !int8_weights &&
        strcmp(precision, "fp16")) {
        fail(error, error_size,
...
static int configure_coreml_qkv(h3_dit *dit, h3_dit_block *block,
    ...
    const char *heads_value = getenv("H3_COREML_QKV_HEADS");
    if (heads_value && *heads_value) {
        char *tail = NULL;
        unsigned long parsed = strtoul(heads_value, &tail, 10);
...
static int configure_coreml_mlp(h3_dit *dit, h3_dit_block *block,
    ...
    const char *gpu_fc1 = getenv("H3_COREML_ANE_GPU_FC1");
    const char *gpu_fc2 = getenv("H3_COREML_ANE_GPU_FC2");
    const char *ane_fc1 = NULL;
    const char *ane_fc2 = NULL;
    const char *manifest = getenv("H3_COREML_ANE_MANIFEST");
    if (directory && *directory) {
        int model_count = snprintf(model_path, sizeof(model_path),
                                   "%s/block-%u.mlpackage", directory, index);
        int fc1_count = snprintf(gpu_fc1_path, sizeof(gpu_fc1_path),
    ...
    const char *width = getenv("H3_COREML_ANE_INTERMEDIATE");
    if (width && *width) {
        char *tail = NULL;
    ...
    const char *output_scale_value = getenv(output_scale_name);
    if (!output_scale_value || !*output_scale_value)
        output_scale_value = getenv("H3_COREML_ANE_OUTPUT_SCALE");
    if (output_scale_value && *output_scale_value) {
        char *tail = NULL;
        ...
        if (tail == output_scale_value || *tail ||
            ...
            fail(error, error_size,
                 ...
                 getenv(output_scale_name) ? output_scale_name :
...
static unsigned final_eviction_interval(void) {
    const char *value = getenv("H3_FINAL_EVICT_BLOCKS");
...
static int profile_steps_enabled(void) {
    const char *value = getenv("H3_PROFILE_STEPS");
    return value && *value && strcmp(value, "0");
}
...
static int configure_explicit_gate_skip(h3_dit *dit, char *error,
                                        size_t error_size) {
    const char *value = getenv("H3_DIT_GATE_SKIP");
    if (!value || !*value) return 1;
...
static int configure_step_gate_skip(h3_dit *dit,
                                    ...
                                    char *error, size_t error_size) {
    const char *value = getenv("H3_DIT_STEP_GATE_SKIP");
    if (!value || !*value) return 1;
...
static unsigned next_private_ane_block(const h3_dit *dit, unsigned current,
    ...
    int use_step_gate_skip = dit->step_gate_skip &&
        !getenv("H3_STEP_GATE_RUNTIME_DISABLE");
...
static int run_private_ane_mlp(h3_dit *dit, h3_dit_block *weight,
    ...
    int enable_fused_int8_epilogue =
        getenv("H3_ENABLE_PRIVATE_ANE_FUSED_INT8_EPILOGUE") != NULL;
    int use_fused_epilogue = !row_split && fuse_next_attention &&
        ...
        !getenv("H3_DISABLE_PRIVATE_ANE_FUSED_EPILOGUE");
    memset(h3_gpu_tensor_host_pointer(dit->ane_nonfinite), 0,
           2u * sizeof(uint32_t));
    ...
    if (range_violation) {
        ...
        int retry_enabled =
            getenv("H3_DISABLE_PRIVATE_ANE_MLP_RANGE_RETRY") == NULL;
...
static int prepare_gate_cache_identity(h3_dit *dit,
                                       const h3_sigma_schedule *sigmas) {
    const char *path = getenv("H3_DIT_GATE_CACHE");
    const char *policy = getenv("H3_DIT_LAYER_POLICY");
...
static void profile_step_gate_scores(const h3_dit *dit) {
    if (!getenv("H3_PROFILE_STEP_GATES")) return;
    int steps = h3_dit_schedule_steps(dit->schedule);
...
static int load_core(h3_dit *dit, h3_dit_progress progress, void *opaque,
    ...
    if (dit->coreml_same_loaded_ab) {
        const char *value = getenv("H3_BENCH_COREML_CAPTURE_BLOCK");
        if (value && *value) {
            char *tail = NULL;
            unsigned long parsed = strtoul(value, &tail, 10);
            if (tail == value || *tail || parsed >= H3_DIT_BLOCKS) {
...
static int finish_coreml_loads(h3_dit *dit,
    ...
    unsigned warmups = 0;
    const char *value = getenv("H3_COREML_WARMUPS");
    if (value && *value) {
        char *tail = NULL;
        unsigned long parsed = strtoul(value, &tail, 10);
...
static int eager_coreml_load_wait(h3_dit *dit,
    ...
    if (!finish_private_ane_attention_out_loads(
            dit, error, error_size)) return 0;
    const char *value = getenv("H3_COREML_LAZY_LOAD");
...
static h3_dit *load_dit(const char *weight_directory,
    ...
    if (!h3_coreml_mlp_configure_fallback(
            getenv("H3_COREML_GPU_FALLBACK"),
            getenv("H3_COREML_FORCE_GPU_FALLBACK"),
            getenv("H3_COREML_FALLBACK_LATCH"),
    ...
    dit->coreml_same_loaded_ab =
        getenv("H3_BENCH_COREML_SAME_LOADED_AB") != NULL;
    if (dit->coreml_same_loaded_ab &&
    ...
    if (dit->first_block_cache && getenv("H3_PROFILE"))
        fprintf(stderr,
    ...
    if (dit->tea_cache && getenv("H3_PROFILE"))
        fprintf(stderr,
    ...
    if (dit->sol_attention && getenv("H3_PROFILE"))
        fprintf(stderr,
                ...
                dit->sol_dense_final_steps,
                getenv("H3_SOL_FULL_SINK") ? " full-sink/all-exact" : "");
    ...
    if (private_ane_mlp_enabled()) {
        const char *gpu_precision =
            getenv("H3_PRIVATE_ANE_MLP_GPU_PRECISION");
...
static int diagnose_coreml_range_block(unsigned index) {
    const char *value = getenv("H3_DIAG_COREML_RANGE_BLOCK");
...
static int dump_mlp_input_requested(int step, char *error,
                                    size_t error_size) {
    const char *directory = getenv("H3_DUMP_MLP_INPUTS_DIR");
    if (!directory || !*directory) return 0;
    int requested_step = 0;
    const char *value = getenv("H3_DUMP_MLP_INPUT_STEP");
    if (value && *value) {
        char *tail = NULL;
        long parsed = strtol(value, &tail, 10);
        if (tail == value || *tail || parsed < 0 || parsed >= H3_MAX_STEPS) {
...
static int dump_qkv_input_requested(int step, unsigned block,
                                    char *error, size_t error_size) {
    const char *directory = getenv("H3_DUMP_QKV_INPUTS_DIR");
    if (!directory || !*directory) return 0;
    int requested_step = 0;
    const char *value = getenv("H3_DUMP_QKV_INPUT_STEP");
    if (value && *value) {
        char *tail = NULL;
        long parsed = strtol(value, &tail, 10);
        if (tail == value || *tail || parsed < 0 || parsed >= H3_MAX_STEPS) {
    ...
    const char *blocks = getenv("H3_DUMP_QKV_INPUT_BLOCKS");
    if (!blocks || !*blocks) return 1;
    return coreml_block_in_list(
...
static int dump_attention_output_input_requested(
                                    ...
                                    char *error, size_t error_size) {
    const char *directory = getenv("H3_DUMP_ATTN_OUT_INPUTS_DIR");
    if (!directory || !*directory) return 0;
    int requested_step = 0;
    const char *value = getenv("H3_DUMP_ATTN_OUT_INPUT_STEP");
    if (value && *value) {
        char *tail = NULL;
        long parsed = strtol(value, &tail, 10);
        if (tail == value || *tail || parsed < 0 || parsed >= H3_MAX_STEPS) {
    ...
    if (step != requested_step) return 0;
    const char *blocks = getenv("H3_DUMP_ATTN_OUT_INPUT_BLOCKS");
    if (!blocks || !*blocks) return 1;
    return coreml_block_in_list(
...
static int run_block(h3_dit *dit, unsigned index, int step,
    ...
    const char *profile_ops_value = getenv("H3_PROFILE_OPS");
...
#define OP(call, label) do {                                                    \
    ...
    if (weight->ane_qkv && rows == dit->sequence &&
        !getenv("H3_DISABLE_PRIVATE_ANE_QKV")) {
    ...
    if (use_sol_bq64_exact || use_sol_bq64_sparse) {
        ...
        if (use_sol_bq64_sparse) {
            ...
            uint32_t sink_end = getenv("H3_SOL_FULL_SINK") ?
    ...
    if (use_sol_attention && getenv("H3_SOL_ROUTE_PROFILE")) {
    ...
    const char *dump_mlp_blocks = getenv("H3_DUMP_MLP_INPUT_BLOCKS");
...
#define OP(call, label) do {                                                    \
    ...
    int use_first_block_cache = dit->first_block_cache &&
        !getenv("H3_FBC_RUNTIME_DISABLE");
    int use_tea_cache = dit->tea_cache &&
        !getenv("H3_TEACACHE_RUNTIME_DISABLE");
    int use_step_gate_skip = dit->step_gate_skip &&
        !getenv("H3_STEP_GATE_RUNTIME_DISABLE");
    ...
    if (evaluate_core) {
            ...
            int cooldown = dit->tea_cache_cooldown_steps &&
                (unsigned)step >= steps -
                    (dit->tea_cache_cooldown_steps < steps ?
                        dit->tea_cache_cooldown_steps : steps);
            int forced_fresh = getenv("H3_TEACACHE_FORCE_FRESH") != NULL;
        ...
        for (unsigned block = 0; block < H3_DIT_BLOCKS; block++) {
            ...
            if (use_first_block_cache && block == first_cache_block) {
                int last_step =
                    step == h3_dit_schedule_steps(dit->schedule) - 1;
                int forced_fresh = getenv("H3_FBC_FORCE_FRESH") != NULL;
...
void h3_dit_free(h3_dit *dit) {
    ...
    for (unsigned block = 0; block < H3_DIT_BLOCKS; block++) {
        ...
        if (value->coreml_mlp &&
            (getenv("H3_PROFILE") || value->coreml_fallbacks))
            fprintf(stderr,
        ...
        if (value->coreml_qkv && getenv("H3_PROFILE"))
            fprintf(stderr,
        ...
        if (value->ane_qkv &&
            (getenv("H3_PROFILE") || value->ane_qkv_prediction_failures ||
        ...
        if (value->ane_mlp &&
            (getenv("H3_PROFILE") || value->ane_prediction_failures ||
    ...
    if (dit->final_eviction_groups && getenv("H3_PROFILE"))
        fprintf(stderr,
