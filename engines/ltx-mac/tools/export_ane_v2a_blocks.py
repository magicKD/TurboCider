#!/usr/bin/env python3
"""Batch-export Stage-1/Stage-2 LTX V-to-A Core ML artifacts."""
...
def main() -> None:
    ...
    exporter = Path(__file__).with_name("export_ane_v2a_artifact.py")
    for completed, block in enumerate(args.blocks, 1):
        for stage, rows in (("stage1", 1001), ("stage2", 4004)):
            ...
            command = [
                ...
                "--video-rows", str(rows), "--variants", args.variant,
                "--compile", "--compiled-only",
            ]
            if args.force:
                command.append("--force")
            print(
                f"[{completed}/{len(args.blocks)}] block {block} {stage}",
                flush=True,
            )
